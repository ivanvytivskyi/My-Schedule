...
function toggleCooking(checkbox) {
    ...
    // NEW: Get the scaled recipe data if it exists (stored as JSON string in data attribute)
    let recipe;
    const scaledRecipeJson = checkbox.dataset.scaledRecipe;
    if (scaledRecipeJson && scaledRecipeJson !== 'null' && scaledRecipeJson !== 'undefined') {
        try {
            recipe = JSON.parse(scaledRecipeJson);
            console.log(`🔄 Using SCALED recipe for cooking (serves ${recipe.serves})`);
        } catch (e) {
            console.warn('Failed to parse scaled recipe, using original:', e);
    ...
    if (isCooked) {
        ...
        // Show scaled quantity in toast if applicable
        const servesInfo = recipe.isScaled ? ` (${recipe.serves} servings)` : '';
        showToast(`🍳 Cooked: ${recipeName}${servesInfo}`);
    } else {
...
// NEW: Now accepts scaledRecipe as 4th parameter
function getCookingCheckboxHTML(blockId, recipeId, recipeName, scaledRecipe = null) {
    const isCooked = isBlockCooked(blockId);
    
    // Convert scaledRecipe to JSON string for storage in data attribute
    const scaledRecipeData = scaledRecipe ? JSON.stringify(scaledRecipe) : 'null';
    
    // Show scaling indicator in the checkbox label if recipe is scaled
    let servesLabel = '';
    if (scaledRecipe && scaledRecipe.isScaled) {
        servesLabel = ` (${scaledRecipe.serves} servings)`;
    }
    ...
    return `
        ...
        ">
            <input type="checkbox" 
                   ...
                   data-scaled-recipe='${scaledRecipeData}'
                   ${isCooked ? 'checked' : ''}
                   onchange="toggleCooking(this)"
                   style="cursor: pointer; margin: 0;">
            <span style="color: ${isCooked ? '#16a34a' : '#065f46'}; white-space: nowrap;">
                🍳 ${isCooked ? 'Cooked' : 'Cook'}${servesLabel}
            </span>
        </label>
    `;
}

console.log('✅ Cooking Integration System loaded - V2.1.2 (Recipe Scaling FIXED)');
