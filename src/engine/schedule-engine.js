...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Auto-cleanup expired medicine blocks
    const removedCount = cleanupExpiredMedicineBlocks();
    if (removedCount > 0) {
        console.log(`🧹 Startup cleanup: Removed ${removedCount} expired medicine blocks`);
    }
...
function buildPredictedDayBusyIntervals(dayDate, config = {}, defaultBlocks = [], workSchedule = {}, commuteSettings = {}, mealSettings = {}) {
    if (!dayDate) return { busyIntervals: [], blocks: [], dayWindowStart: 0, dayWindowEnd: 24 * 60 };
    const dayName = getDayName(dayDate);
    const dayOfWeek = dayDate.getDay();
    const dayWindowStart = timeStrToMinutes(config.dayWindowStart || scheduleData?.dayWindow?.start || '07:00');
    const dayWindowEnd = timeStrToMinutes(config.dayWindowEnd || scheduleData?.dayWindow?.end || '23:00');
    const useExistingBlocks = !(typeof isSetupWizardActive === 'function' && isSetupWizardActive());
    const existingBlocks = useExistingBlocks && Array.isArray(config.existingBlocks) ? config.existingBlocks : null;
    let blocks = existingBlocks ? existingBlocks.slice() : [];

    const work = workSchedule?.[dayOfWeek];
    const resolvedCommute = commuteSettings && Object.keys(commuteSettings).length
        ? commuteSettings
    ...
    if (!existingBlocks) {
        ...
        defaults.forEach(defaultBlock => {
            ...
            blocks.push(...buildPrepTravelBlocks(copyBlock, dayDate));
            blocks.push(copyBlock);
        });

        if (work && work.start && work.end) {
            const commuteDuration = Number.isFinite(resolvedCommute.commuteDuration) ? resolvedCommute.commuteDuration : 0;
            const commutePrepDuration = Number.isFinite(resolvedCommute.commutePrepDuration) ? resolvedCommute.commutePrepDuration : 0;
            const addCommute = resolvedCommute.addCommute === true;
            const addCommutePrep = resolvedCommute.addCommutePrep === true;
            const workBlockId = `work_preview_${dayDate.getTime()}`;

            if (addCommute && addCommutePrep && commutePrepDuration > 0) {
                const commuteStart = subtractMinutesFromTime(work.start, commuteDuration);
            ...
            if (addCommute && commuteDuration > 0) {
                const commuteStart = subtractMinutesFromTime(work.start, commuteDuration);
                blocks.push({
                    linkedBlockId: workBlockId,
                    time: `${commuteStart}-${work.start}`,
            ...
            if (addCommute && commuteDuration > 0) {
                const commuteEnd = addMinutesToTime(work.end, commuteDuration);
                blocks.push({
                    linkedBlockId: workBlockId,
                    time: `${work.end}-${commuteEnd}`,
                    title: '🚶 Commute home'
                });
            }
        }

        const cookingConfig = mealSettings.cookingConfig || getCookingConfig();
        const maxWorkCookTime = cookingConfig.maxCookingTimeWorkDays || 45;
        const cookingDurations = {
            breakfast: Math.min(15, maxWorkCookTime),
            lunch: Math.min(30, maxWorkCookTime),
            dinner: Math.min(45, maxWorkCookTime)
        };

        const addCookingPlaceholder = (mealBlock, mealType) => {
            const range = getBlockTimeRange(mealBlock);
            if (isNaN(range.start) || isNaN(range.end)) return;
            const cookDur = cookingDurations[mealType] || 20;
            const eatDur = range.end - range.start;
            if (!Number.isFinite(eatDur) || eatDur <= 0) return;

            if (mealType === 'lunch') {
                let cookEnd = range.start;
                let cookStart = Math.max(0, cookEnd - cookDur);
                const intendedEnd = range.end;
                const latestLunchEnd = timeStrToMinutes('15:30');

                const busyIntervals = [];
                blocks.forEach(block => {
                    if (block.isCookingBlock) return;
                    busyIntervals.push(...getBlockIntervalsWithinWindow(block, dayWindowStart, dayWindowEnd));
                });
                const mergedBusy = mergeIntervals(busyIntervals);
                const overlaps = (start, end) =>
                    mergedBusy.some(interval => start < interval.end && end > interval.start);
                const conflict = mergedBusy.find(interval => cookStart < interval.end && cookEnd > interval.start);

                if (conflict) {
                    let cursor = range.start;
                    let placed = false;
                    while (cursor + cookDur + eatDur <= latestLunchEnd) {
                        const windowStart = cursor;
                        const windowEnd = cursor + cookDur + eatDur;
                        if (!overlaps(windowStart, windowEnd)) {
                            cookStart = windowStart;
                            cookEnd = cookStart + cookDur;
                            mealBlock.time = `${formatMinutesToTime(cookEnd)}-${formatMinutesToTime(windowEnd)}`;
                            placed = true;
                            console.log(`🍱 Lunch shifted due to overlap: original=${formatMinutesToTime(range.start - cookDur)}-${formatMinutesToTime(intendedEnd)} conflict=${formatMinutesToTime(conflict.start)}-${formatMinutesToTime(conflict.end)} new=${formatMinutesToTime(cookStart)}-${formatMinutesToTime(windowEnd)}`);
                            break;
                        }
                        const blocking = mergedBusy.find(interval => windowStart < interval.end && windowEnd > interval.start);
                        cursor = blocking ? Math.max(cursor, blocking.end) : cursor + 5;
                    }
                    if (!placed) {
                        mealBlock.removeForWindow = true;
                        return;
                    }
                }

                const finalRange = getBlockTimeRange(mealBlock);
                if (finalRange.end > latestLunchEnd) {
                    mealBlock.removeForWindow = true;
                    return;
                }
        ...
        blocks.forEach(block => {
            ...
            if (isBreakfastBlock(block)) addCookingPlaceholder(block, 'breakfast');
            if (isLunchBlock(block)) addCookingPlaceholder(block, 'lunch');
            if (isDinnerBlock(block)) addCookingPlaceholder(block, 'dinner');
        });
        blocks = blocks.filter(block => !block.removeForWindow);

        const workRange = work && work.start && work.end
            ? {
                start: timeStrToMinutes(work.start),
                end: timeStrToMinutes(work.end)
            }
            : null;
        blocks = applyWorkMealRules(blocks, workRange, null, { allowAutoReschedule: true });
    }
    ...
    const lunchBlock = blocks.find(block => mealTypeOf(block) === 'lunch' && !block.isCookingBlock);
    if (lunchBlock) {
        const { start, end } = getBlockTimeRange(lunchBlock);
        if (!isNaN(start) && !isNaN(end)) {
            console.log(`🥪 Lunch interval included: ${formatMinutesToTime(start)}-${formatMinutesToTime(end)}`);
        }
    }
    console.log(`🧭 Busy intervals for ${dayName}:`, merged);
    return { busyIntervals: merged, blocks, dayWindowStart, dayWindowEnd };
}
...
function buildPreviewBlocksForDay(dayName, date, isWorkDay = false) {
    ...
    const cookingDurations = {
        breakfast: Math.min(15, maxWorkCookTime),
        lunch: Math.min(30, maxWorkCookTime),
        dinner: isWorkDay ? Math.min(45, maxWorkCookTime) : 45
    };

    const addCookingPlaceholder = (mealBlock, mealType) => {
        const { start } = getBlockTimeRange(mealBlock);
        if (isNaN(start)) return;
        const duration = cookingDurations[mealType] || 20;
        const cookEnd = start;
        const cookStart = Math.max(0, cookEnd - duration);
        const cookingBlock = {
            time: `${formatMinutesToTime(cookStart)}-${formatMinutesToTime(cookEnd)}`,
            title: `Cook ${mealType.charAt(0).toUpperCase() + mealType.slice(1)}`,
            tasks: ['Cook', 'Wash dishes after cooking', 'Clean workspace'],
            isCookingBlock: true,
            mealType: mealType
        };
        blocks.push(cookingBlock);
    };
...
function setupTitleSuggestions() {
    ...
    titleSuggestBtn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        
        console.log('📋 Title suggestions clicked!');
        
        // Toggle visibility
        const isVisible = titleSuggestions.style.display === 'block';
        ...
        if (!isVisible) {
            ...
            console.log('✅ Showing', commonTitles.length, 'title suggestions');
            ...
            // Hide suggestions
            titleSuggestions.style.display = 'none';
            console.log('❌ Hidden suggestions');
        }
    };
    
    console.log('✅ Title suggestions setup complete');
...
function addSingleDay() {
    ...
    if (scheduleData.defaultBlocks && scheduleData.defaultBlocks.length > 0) {
        scheduleData.defaultBlocks.forEach(defaultBlock => {
            ...
            // MEDICINE CHECK: If this is a medicine block, check date range
            if (defaultBlock.medicineSchedule) {
                const blockDate = new Date(day.date);
                blockDate.setHours(0, 0, 0, 0);
                
                const startDate = new Date(defaultBlock.medicineSchedule.startDate);
                startDate.setHours(0, 0, 0, 0);
                
                const endDate = new Date(defaultBlock.medicineSchedule.endDate);
                endDate.setHours(23, 59, 59, 999);
                
                // Skip if current day is outside medicine date range
    ...
    const finalizedBlocks = finalizeDayBlocks(blocks, date, dayKey);
    scheduleData.days[dayKey] = {
        name: dayName,
        date: date.toISOString().split('T')[0],
        displayDate: date.toLocaleDateString('en-GB', { month: 'short', day: 'numeric' }),
...
async function addWeek(options = {}) {
    console.log('🚀 CHECKPOINT A: addWeek() started');
    const { suppressShoppingModal = false } = options;
    ...
    const sharedCommuteInput = document.getElementById('workPatternCommute');
    const sharedPrepInput = document.getElementById('workPatternPrep');
    const sharedCommute = parseInt(sharedCommuteInput?.value || '', 10);
    const sharedPrep = parseInt(sharedPrepInput?.value || '', 10);
    const hasSharedCommute = isSetupWizardActive() && (Number.isFinite(sharedCommute) || Number.isFinite(sharedPrep));

    const usePatternCommute = patternMode === 'same' && patternRows.length;
    const addCommute = hasSharedCommute
        ? sharedCommute > 0
        : (usePatternCommute || document.getElementById('addCommuteNew')?.checked || document.getElementById('addCommute').checked);
    const commuteDurationRaw = hasSharedCommute
        ? sharedCommute
        : (usePatternCommute
            ? patternCommute
            : parseInt(document.getElementById('commuteDurationNew')?.value || document.getElementById('commuteDuration')?.value || '0', 10));
    const commuteDuration = addCommute && Number.isFinite(commuteDurationRaw) ? commuteDurationRaw : 0;
    const addCommutePrep = addCommute && (hasSharedCommute ? sharedPrep > 0 : (usePatternCommute || document.getElementById('addCommutePrepNew')?.checked || false));
    const commutePrepRaw = hasSharedCommute
        ? sharedPrep
        : (usePatternCommute ? patternPrep : parseInt(document.getElementById('commutePrepDurationNew')?.value || '0', 10));
    const commutePrepDuration = addCommutePrep && Number.isFinite(commutePrepRaw) ? commutePrepRaw : 0;
    ...
    for (let i = 0; i < orderedDays.length; i++) {
        ...
        // Add any existing blocks from orderedDays (e.g., shopping blocks)
        if (orderedDays[i].blocks && orderedDays[i].blocks.length > 0) {
            blocks.push(...orderedDays[i].blocks);
        }
        ...
        if (isWorkDay) {
            ...
            blocks = applyWorkMealRules(blocks, workRange, daySummary, { allowAutoReschedule: true });
        } else {
            blocks = applyWorkMealRules(blocks, null, daySummary, { forceOverlapResolution: true, allowAutoReschedule: true });
        }
        ...
        const finalizedBlocks = finalizeDayBlocks(dedupeSleepBlocks(blocks), date, dayKey);
        scheduleData.days[dayKey] = {
            name: dayName,
            date: date.toISOString().split('T')[0],
            displayDate: date.toLocaleDateString('en-GB', { month: 'short', day: 'numeric' }),
    ...
    if (stockAnalysis.needsShoppingRecipes.length > 0) {
        ...
        if (!suppressShoppingModal) {
            ...
            console.log('🛒 Shopping modal suppressed; using setup wizard for shopping selection.');
        }
    } else {
    ...
    try {
        ...
        console.error('❌ Failed to finalize week rendering:', error);
    }
        ...
        html += `
            <div class="day-content" id="${dayKey}Content">
                <div class="day-header">
                    <div class="day-free-time" id="freeTime-${dayKey}">
                        <span>✨Available time today: --</span>
                        <button class="add-block-btn" onclick="openQuickAddBlock('${dayKey}')" title="Add new block" style="margin-left:12px;width:40px;height:40px;padding:0;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;border:none;border-radius:10px;font-size:20px;font-weight:700;cursor:pointer;box-shadow:0 4px 12px rgba(102,126,234,0.3);vertical-align:middle;transition:all 0.3s ease;display:inline-flex;align-items:center;justify-content:center;" onmouseover="this.style.transform='translateY(-2px) scale(1.05)';this.style.boxShadow='0 6px 16px rgba(102,126,234,0.4)';" onmouseout="this.style.transform='translateY(0) scale(1)';this.style.boxShadow='0 4px 12px rgba(102,126,234,0.3)';">➕</button>
                    </div>
...
function updateAllDayFreeTime() {
    for (const dayKey in scheduleData.days) {
        ...
        // Update only the span, not the whole div (to preserve button)
        const span = el.querySelector('span');
        if (span) {
            span.textContent = `✨Available time today: ${formatMinutesAsHoursAndMinutes(freeMins)}`;
        } else {
        ...
        if (isCooking && block.recipeID && !block.isLeftover && typeof getCookingCheckboxHTML === 'function') {
            html += `
                <div class="task-item cook-action">
                    ${getCookingCheckboxHTML(blockId, block.recipeID, block.recipeName || 'Recipe', block.scaledRecipe)}
                </div>
            `;
        }
...
// ==============================================
// OLD MEDICINE HELPER FUNCTIONS (NO LONGER USED)
// Medicine form now appears in Default Blocks Manager only
// ==============================================
// Medicine Schedule Helper - Auto-create multiple blocks
function addMedicineScheduleHelper() {
    const titleInput = document.getElementById('blockTitle');
    if (!titleInput) return;
    
    // Check if helper already exists
    let helperDiv = document.getElementById('medicineScheduleHelper');
    if (!helperDiv) {
        helperDiv = document.createElement('div');
        helperDiv.id = 'medicineScheduleHelper';
        helperDiv.style.cssText = 'display:none;margin-top:15px;padding:15px;background:#fef3c7;border:2px solid #fbbf24;border-radius:8px;';
        helperDiv.innerHTML = `
            <div style="font-weight:700;color:#78350f;margin-bottom:10px;font-size:14px;">💊 Medicine Schedule Auto-Fill</div>
            <div style="margin-bottom:10px;">
                <label style="display:block;font-size:12px;color:#78350f;margin-bottom:4px;">Medicine Name:</label>
                <input type="text" id="medicineName" placeholder="e.g., Aspirin" style="width:100%;padding:8px;border:1px solid #fbbf24;border-radius:6px;font-size:13px;">
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                <div>
                    <label style="display:block;font-size:12px;color:#78350f;margin-bottom:4px;">Times per day:</label>
                    <input type="number" id="medicineTimesPerDay" min="1" max="10" value="2" style="width:100%;padding:8px;border:1px solid #fbbf24;border-radius:6px;font-size:13px;">
                </div>
                <div>
                    <label style="display:block;font-size:12px;color:#78350f;margin-bottom:4px;">Duration (minutes):</label>
                    <input type="number" id="medicineDuration" min="1" max="60" value="2" style="width:100%;padding:8px;border:1px solid #fbbf24;border-radius:6px;font-size:13px;">
                </div>
            </div>
            <div id="medicineTimesInputs" style="margin-bottom:10px;"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                <div>
                    <label style="display:block;font-size:12px;color:#78350f;margin-bottom:4px;">Start Date:</label>
                    <input type="date" id="medicineStartDate" style="width:100%;padding:8px;border:1px solid #fbbf24;border-radius:6px;font-size:13px;">
                </div>
                <div>
                    <label style="display:block;font-size:12px;color:#78350f;margin-bottom:4px;">End Date:</label>
                    <input type="date" id="medicineEndDate" style="width:100%;padding:8px;border:1px solid #fbbf24;border-radius:6px;font-size:13px;">
                </div>
            </div>
            <button type="button" onclick="generateMedicineBlocks()" style="width:100%;padding:10px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:14px;">
        ...
        if (applyToAllCheckbox) {
            applyToAllCheckbox.parentElement.parentElement.appendChild(helperDiv);
        }
        
        // Listen for times per day changes
        const timesInput = document.getElementById('medicineTimesPerDay');
        if (timesInput) {
            timesInput.addEventListener('change', updateMedicineTimeInputs);
            updateMedicineTimeInputs(); // Initial render
        }
        ...
        document.getElementById('medicineStartDate').value = today.toISOString().split('T')[0];
        document.getElementById('medicineEndDate').value = nextWeek.toISOString().split('T')[0];
    }
    
    // Show/hide based on title
    titleInput.addEventListener('input', () => {
        checkAndShowMedicineHelper();
    });
}

// Check if title is medicine-related and show helper
function checkAndShowMedicineHelper() {
    const titleInput = document.getElementById('blockTitle');
    const helperDiv = document.getElementById('medicineScheduleHelper');
    
    if (!titleInput || !helperDiv) return;
    
    const title = titleInput.value.toLowerCase();
    const isMedicine = title.includes('medicine') || title.includes('medication') || 
                      title.includes('pill') || title.includes('drug') || title.includes('💊');
...
function updateMedicineTimeInputs() {
    const timesPerDay = parseInt(document.getElementById('medicineTimesPerDay')?.value || 2);
    const container = document.getElementById('medicineTimesInputs');
    if (!container) return;
    ...
    for (let i = 1; i <= timesPerDay; i++) {
        ...
        const defaultTime = defaultTimes[i - 1] || '12:00';
        
        const timeDiv = document.createElement('div');
        timeDiv.style.cssText = 'margin-bottom:8px;';
        timeDiv.innerHTML = `
            <label style="display:block;font-size:12px;color:#78350f;margin-bottom:4px;">Dose ${i} - Time:</label>
            <input type="time" class="medicine-time-input" value="${defaultTime}" style="width:100%;padding:8px;border:1px solid #fbbf24;border-radius:6px;font-size:13px;">
        `;
        container.appendChild(timeDiv);
    }
}

// Generate medicine schedule as DEFAULT BLOCKS
window.generateMedicineDefaultBlocks = function() {
    const medicineName = document.getElementById('medicineName').value;
    const timesPerDay = parseInt(document.getElementById('medicineTimesPerDay').value);
    const duration = parseInt(document.getElementById('medicineDuration').value);
    const startDate = new Date(document.getElementById('medicineStartDate').value);
    const endDate = new Date(document.getElementById('medicineEndDate').value);
    const times = Array.from(document.querySelectorAll('.medicine-time-input')).map(input => input.value);
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
        alert('⚠️ Please select valid dates');
        return;
    }
    ...
    const confirm = window.confirm(
        ...
        `Days: ${daysDiff}\n\n` +
        `This will create ${timesPerDay} default blocks that will appear on all days until ${endDate.toLocaleDateString()}!`
    );
    
    if (!confirm) return;
    
    // Create default blocks (one for each time of day)
    times.forEach((time, index) => {
        const startMins = timeStrToMinutes(time);
        ...
        const medicineBlock = {
            time: `${time}-${endTime}`,
            title: `💊 ${medicineName}`,
            tasks: [`Dose ${index + 1} of ${timesPerDay}`],
        ...
        scheduleData.defaultBlocks.push(medicineBlock);
    });
    
    // Close medicine modal, save, and render
    closeMedicineScheduleModal();
    saveToLocalStorage();
    renderSchedule();
    
    // CRITICAL: Refresh the default blocks list so user can see the new blocks
    if (typeof renderDefaultBlocksList === 'function') {
        renderDefaultBlocksList();
        console.log('✅ Default blocks list refreshed');
    }
    
    alert(
        `✅ Created ${timesPerDay} medicine default blocks!\n\n` +
        `They will appear on all days from ${startDate.toLocaleDateString()} to ${endDate.toLocaleDateString()}.\n\n` +
        `Blocks will be automatically removed after ${endDate.toLocaleDateString()}.`
    );
};

// OLD FUNCTION - Kept for backwards compatibility but not used
window.generateMedicineBlocks = function() {
    const medicineName = document.getElementById('medicineName').value;
    const timesPerDay = parseInt(document.getElementById('medicineTimesPerDay').value);
    const duration = parseInt(document.getElementById('medicineDuration').value);
    const startDate = new Date(document.getElementById('medicineStartDate').value);
    ...
    // Calculate number of days
    const daysDiff = Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
    const totalBlocks = daysDiff * timesPerDay;
    ...
    const confirm = window.confirm(
        ...
        `To: ${endDate.toLocaleDateString()}\n` +
        `Days: ${daysDiff}\n\n` +
        `This will create ${totalBlocks} important blocks!`
    );
    ...
    while (currentDate <= endDate) {
        ...
        if (dayKey) {
            times.forEach((time, index) => {
                ...
                scheduleData.days[dayKey].blocks.push(medicineBlock);
                splitOverlappingBlocks(dayKey, medicineBlock);
                blocksCreated++;
            });
        }
    ...
    // Close modal, save, and render
    closeModal();
    saveToLocalStorage();
    renderSchedule();
    
    alert(`✅ Created ${blocksCreated} medicine blocks!\n\nSchedule generated successfully.\n\nBlocks will be automatically removed after ${endDate.toLocaleDateString()}.`);
};

// Auto-remove expired medicine blocks
function cleanupExpiredMedicineBlocks() {
    const today = new Date();
    ...
    Object.keys(scheduleData.days || {}).forEach(dayKey => {
        ...
        // Filter out expired medicine blocks
        const originalLength = day.blocks.length;
        day.blocks = day.blocks.filter(block => {
            // Check if block is a medicine block with schedule metadata
            if (block.medicineSchedule && block.medicineSchedule.endDate) {
                const endDate = new Date(block.medicineSchedule.endDate);
                endDate.setHours(23, 59, 59, 999); // End of that day
                
                // Remove if expired
                if (endDate < today) {
                    console.log(`🗑️ Removing expired medicine from ${dayKey}: ${block.title} (ended ${block.medicineSchedule.endDate})`);
        ...
        removedCount += (originalLength - day.blocks.length);
    });
    
    // Clean up DEFAULT blocks
    if (scheduleData.defaultBlocks && Array.isArray(scheduleData.defaultBlocks)) {
        const originalLength = scheduleData.defaultBlocks.length;
        scheduleData.defaultBlocks = scheduleData.defaultBlocks.filter(block => {
            // Check if block is a medicine block with schedule metadata
            if (block.medicineSchedule && block.medicineSchedule.endDate) {
                const endDate = new Date(block.medicineSchedule.endDate);
                endDate.setHours(23, 59, 59, 999); // End of that day
                ...
                // Remove if expired
                if (endDate < today) {
                    console.log(`🗑️ Removing expired DEFAULT medicine: ${block.title} (ended ${block.medicineSchedule.endDate})`);
                    return false; // Remove this block
                }
            }
            return true; // Keep this block
        ...
        removedCount += (originalLength - scheduleData.defaultBlocks.length);
    }
    
    if (removedCount > 0) {
        console.log(`✅ Cleaned up ${removedCount} expired medicine blocks`);
        saveToLocalStorage();
        return removedCount;
    }
...
// Split overlapping blocks when important block is added
function splitOverlappingBlocks(dayKey, importantBlock) {
    const day = scheduleData.days[dayKey];
    if (!day || !day.blocks) return;
    ...
    console.log(`🔪 Checking overlaps for important block: ${importantBlock.title} (${impStart}-${impEnd})`);
    
    // Find blocks that overlap with this important block
    const overlappingBlocks = [];
    day.blocks.forEach((block, index) => {
        if (block === importantBlock) return; // Skip self
        if (block.canSplit || block.blockType === 'important') return; // Skip other important blocks
        ...
        if (startMins < impEndMins && endMins > impStartMins) {
            console.log(`  📍 Found overlap: ${block.title} (${start}-${end})`);
            overlappingBlocks.push({ block, index });
        }
    });
    ...
    // Split each overlapping block
    // We need to process them in reverse order (highest index first)
    // to avoid index shifting issues when we splice
    overlappingBlocks.sort((a, b) => b.index - a.index);
    
    overlappingBlocks.forEach(({ block, index }) => {
        const [overStart, overEnd] = block.time.split('-');
        const overStartMins = timeStrToMinutes(overStart);
        const overEndMins = timeStrToMinutes(overEnd);
        
        console.log(`  🔪 Splitting: ${block.title}`);
        
        // Remove the original block
        day.blocks.splice(index, 1);
        
        // Calculate the split parts
        const beforeExists = overStartMins < impStartMins;
        const afterExists = overEndMins > impEndMins;
        
        // Add part BEFORE important block (if exists)
        if (beforeExists) {
            const beforeBlock = {
                ...block,
                time: `${overStart}-${impStart}`,
                startMins: overStartMins,
                endMins: impStartMins
            };
            day.blocks.push(beforeBlock);
            console.log(`    ➕ Added BEFORE: ${overStart}-${impStart}`);
        }
        
        // Important block stays in the middle
        
        // Add part AFTER important block (if exists)
        if (afterExists) {
            const afterBlock = {
                ...block,
                time: `${impEnd}-${overEnd}`,
                startMins: impEndMins,
                endMins: overEndMins
            };
            day.blocks.push(afterBlock);
            console.log(`    ➕ Added AFTER: ${impEnd}-${overEnd}`);
        }
    });
    
    // Re-sort blocks by time
    day.blocks.sort((a, b) => {
...
document.getElementById('editForm').addEventListener('submit', (e) => {
    ...
    // Check if this is an important block (from pending metadata or existing block)
    const pendingMeta = window._pendingBlockMetadata || {};
    const existingBlock = (currentEditingBlock !== null && currentEditingBlock >= 0 && scheduleData.days[currentEditingDay]?.blocks?.[currentEditingBlock])
        ...
        : {};
    const isImportantBlock = pendingMeta.canSplit || existingBlock.canSplit || 
                            pendingMeta.blockType === 'important' || existingBlock.blockType === 'important';

    // Check for time conflicts (skip for important blocks - they can overlap)
    if (!isImportantBlock && checkTimeConflict(currentEditingDay, time, currentEditingBlock >= 0 ? currentEditingBlock : -1)) {
    ...
    const newBlock = {
        ...
        // Add metadata for important blocks
        ...(pendingMeta.blockType && { blockType: pendingMeta.blockType }),
        ...(pendingMeta.canSplit !== undefined && { canSplit: pendingMeta.canSplit }),
        ...(pendingMeta.hasPrepTravel && { 
            hasPrepTravel: pendingMeta.hasPrepTravel,
            prepDuration: pendingMeta.prepDuration,
            travelDuration: pendingMeta.travelDuration
        })
    };
    
    // Clear pending metadata after using it
    if (window._pendingBlockMetadata) {
        delete window._pendingBlockMetadata;
    }
    ...
    // CRITICAL: If this is an important block, split any overlapping blocks
    if (newBlock.canSplit || newBlock.blockType === 'important') {
        console.log('🔪 Important block detected, checking for overlaps to split');
        splitOverlappingBlocks(currentEditingDay, newBlock);
    }
...
function applyWorkMealRules(blocks, workRange, daySummary, options = {}) {
    ...
    const findLunchPlacement = (startMins, duration, busy, latestEnd) => {
        const merged = mergeBusyIntervals(busy);
        let cursor = Math.max(0, startMins);
        for (const interval of merged) {
            if (interval.end <= cursor) continue;
            if (interval.start - cursor >= duration) {
                const end = cursor + duration;
                if (end <= latestEnd) return { start: cursor, end };
                return null;
            }
            cursor = Math.max(cursor, interval.end);
            if (cursor + duration > latestEnd) return null;
        }
        const finalEnd = cursor + duration;
        if (finalEnd <= latestEnd) return { start: cursor, end: finalEnd };
        return null;
    };

    const findArrivalHomeMinutes = () => {
        let arrival = workWindow ? workWindow.end : NaN;
        blocks.forEach(block => {
            const title = (block.title || '').toLowerCase();
            const isCommuteHome = title.includes('commute home') || title.includes('commute from work');
            const isWorkChainHome = block.memberRole === 'travelHome' && block.groupId && block.groupId.startsWith('work:');
            if (!isCommuteHome && !isWorkChainHome) return;
            const { end } = getBlockTimeRange(block);
            if (!isNaN(end) && (isNaN(arrival) || end > arrival)) {
                arrival = end;
            }
        });
        if (!isNaN(arrival)) {
            arrival = Math.min(arrival, 24 * 60);
        }
        return arrival;
    };
    ...
    ['lunch', 'dinner'].forEach(type => {
        ...
        const mealRange = getBlockTimeRange(mealBlock);
        if (isNaN(mealRange.start) || isNaN(mealRange.end)) {
            state.remove = true;
        ...
        if (type === 'lunch') {
            ...
            const latestLunchEnd = timeStrToMinutes('15:30');
            const lunchStart = mealRange.start;
            const lunchEnd = mealRange.end;
            const cookStart = lunchStart - cookingDuration;
            const cookEnd = lunchStart;
            const packageDuration = cookingDuration + mealDuration;

            const hasValidDurations = Number.isFinite(mealDuration) && mealDuration > 0;
            if (!hasValidDurations || isNaN(lunchStart) || isNaN(lunchEnd)) {
                state.remove = true;
                record(type, 'removed', 'INVALID_MEAL_TIME');
                console.log(`🥪 Lunch decision: lunchStart=-- lunchEnd=-- decision=REMOVE reason=INVALID_TIME`);
                return;
            }
            ...
            const conflictInterval = busyIntervals.find(interval => cookStart < interval.end && lunchEnd > interval.start);
            if (conflictInterval) {
                const placement = findLunchPlacement(cookStart, packageDuration, busyIntervals, latestLunchEnd);
                if (!placement) {
                    state.remove = true;
                    record(type, 'removed', 'NO_FIT');
                    console.log(`🥪 Lunch shift: original=${formatMinutesToTime(cookStart)}-${formatMinutesToTime(lunchEnd)} conflict=${formatMinutesToTime(conflictInterval.start)}-${formatMinutesToTime(conflictInterval.end)} new=-- decision=REMOVE`);
                    return;
                }
                const shiftedCookStart = placement.start;
                const shiftedCookEnd = shiftedCookStart + cookingDuration;
                const shiftedLunchStart = shiftedCookEnd;
                const shiftedLunchEnd = shiftedLunchStart + mealDuration;
                if (shiftedLunchEnd > latestLunchEnd) {
                    state.remove = true;
                    record(type, 'removed', 'AFTER_CUTOFF');
                    console.log(`🥪 Lunch shift: original=${formatMinutesToTime(cookStart)}-${formatMinutesToTime(lunchEnd)} conflict=${formatMinutesToTime(conflictInterval.start)}-${formatMinutesToTime(conflictInterval.end)} new=${formatMinutesToTime(shiftedCookStart)}-${formatMinutesToTime(shiftedLunchEnd)} decision=REMOVE`);
                    return;
                }
                if (cookingBlock) {
                    if (mealBlock.id) cookingBlock.linkedBlockId = mealBlock.id;
                    if (mealBlock.groupId) cookingBlock.groupId = mealBlock.groupId;
                    updateBlockTimes(cookingBlock, shiftedCookStart, shiftedCookEnd);
                }
                updateBlockTimes(mealBlock, shiftedLunchStart, shiftedLunchEnd);
                record(type, 'kept', 'KEPT_NO_OVERLAP');
                console.log(`🥪 Lunch shift: original=${formatMinutesToTime(cookStart)}-${formatMinutesToTime(lunchEnd)} conflict=${formatMinutesToTime(conflictInterval.start)}-${formatMinutesToTime(conflictInterval.end)} new=${formatMinutesToTime(shiftedCookStart)}-${formatMinutesToTime(shiftedLunchEnd)} decision=KEEP`);
                return;
            }

            if (lunchEnd > latestLunchEnd) {
                state.remove = true;
                record(type, 'removed', 'AFTER_CUTOFF');
                console.log(`🥪 Lunch decision: lunchStart=${formatMinutesToTime(lunchStart)} lunchEnd=${formatMinutesToTime(lunchEnd)} decision=REMOVE reason=AFTER_CUTOFF`);
                return;
            }
            ...
            if (cookingBlock) {
                if (mealBlock.id) cookingBlock.linkedBlockId = mealBlock.id;
                if (mealBlock.groupId) cookingBlock.groupId = mealBlock.groupId;
                updateBlockTimes(cookingBlock, cookStart, lunchStart);
            }
            updateBlockTimes(mealBlock, lunchStart, lunchEnd);
            record(type, 'kept', 'KEPT_NO_OVERLAP');
            console.log(`🥪 Lunch decision: lunchStart=${formatMinutesToTime(lunchStart)} lunchEnd=${formatMinutesToTime(lunchEnd)} decision=KEEP reason=ON_SCHEDULE`);
            return;
        }

        const { start } = mealRange;
        const saneTime = type === 'lunch'
            ? (start >= timeStrToMinutes('11:00') && start <= timeStrToMinutes('15:00'))
            : (start >= timeStrToMinutes('17:00') && start <= timeStrToMinutes('21:30'));
...
function subtractMinutesFromTime(timeStr, minutes) {
    ...
    if (isNaN(base)) return '00:00';
    const total = Math.max(0, base - Number(minutes || 0));
...
function isWorkCommuteBlock(block) {
    const title = (block.title || '').toLowerCase();
    if (!title.includes('commute')) return false;
    return title.includes('work') ||
        title.includes('commute to work') ||
        title.includes('commute from work') ||
        title.includes('commute home') ||
        title.includes('commute prep');
}

function isLinkedToFixedBlock(block, blocks) {
    if (!block || !Array.isArray(blocks)) return false;
    const baseId = block.linkedBlockId || block.id;
    if (!baseId) return false;
    const chainBlocks = blocks.filter(item =>
        item?.id === baseId || item?.linkedBlockId === baseId
    );
    return chainBlocks.some(item =>
        item.fixedTime === true || isWorkBlock(item) || isStudyBlock(item)
    );
}

function isFixedTimeBlock(block, blocks = []) {
    if (!block) return false;
    if (block.fixedTime === true) return true;
    if (isWorkBlock(block)) return true;
    if (isStudyBlock(block)) return true;
    if (isWorkCommuteBlock(block)) return true;
    if (isLinkedToFixedBlock(block, blocks)) return true;
    return false;
}

function isMealOrCookingBlock(block) {
    return Boolean(mealTypeOf(block) || block?.isCookingBlock);
}

function isPackingRoutineBlock(block) {
    const title = (block.title || '').toLowerCase();
...
function orderChainBlocks(chainBlocks, anchorBlock) {
    return chainBlocks.slice().sort((a, b) => {
...
function packLinkedChains(blocks, date) {
    ...
    chainMap.forEach((chainBlocks, chainKey) => {
        ...
        if (chainKey.startsWith('linked:')) {
            anchorBlock = blocks.find(block => block?.id && chainKey === `linked:${block.id}`);
        } else if (chainKey.startsWith('meal:')) {
            anchorBlock = uniqueBlocks.find(block => mealTypeOf(block) && !block.isCookingBlock) || null;
        }
        ...
        let desiredChainStart = ordered.reduce((min, block) => {
            const { start } = getBlockTimeRange(block);
            return isNaN(start) ? min : Math.min(min, start);
        }, Infinity);
        if (anchorBlock) {
            const anchorStart = getBlockTimeRange(anchorBlock).start;
            const anchorIndex = ordered.indexOf(anchorBlock);
            ...
            if (!isNaN(anchorStart)) {
                desiredChainStart = anchorStart - durationBeforeAnchor;
            }
        }
        if (!isFinite(desiredChainStart)) return;
        let cursor = desiredChainStart;
        ordered.forEach(block => {
            const duration = getBlockDurationMinutes(block);
            updateBlockTimesForDate(block, cursor, cursor + duration, date);
            cursor += duration;
        });
    });
...
function applyPriorityNudges(blocks, date, options = {}) {
    ...
    const chainSnapshots = chains.map(chain => {
        ...
        const fixed = chain.blocks.some(block => isFixedTimeBlock(block, blocks));
        const hasMealChain = chain.blocks.some(block => isMealOrCookingBlock(block));
        return { ...chain, start, end, fixed, hasMealChain };
    ...
    chainSnapshots.forEach(chain => {
        if (cursor !== null && chain.start < cursor) {
            ...
            if (!chain.fixed && (shiftBy <= maxShiftMinutes || chain.hasMealChain)) {
                ...
                chain.blocks.forEach(block => {
                    if (!isFixedTimeBlock(block, blocks) && !isMealOrCookingBlock(block) && !isPackingRoutineBlock(block)) {
                        conflicts.add(block);
                    }
                });
            }
    ...
    for (let i = 0; i < sortedBlocks.length; i += 1) {
        ...
        for (let j = i + 1; j < sortedBlocks.length; j += 1) {
            ...
            if (current.start < next.end && current.end > next.start) {
                if (!isFixedTimeBlock(current.block, blocks) && !isMealOrCookingBlock(current.block) && !isPackingRoutineBlock(current.block)) {
                    conflicts.add(current.block);
                }
                if (!isFixedTimeBlock(next.block, blocks) && !isMealOrCookingBlock(next.block) && !isPackingRoutineBlock(next.block)) {
                    conflicts.add(next.block);
                }
            }
        }
    }
...
function enforceCookBeforeMealWithPull(blocks, dayKey) {
    if (!Array.isArray(blocks) || blocks.length === 0) return blocks;
    const isFixedBlock = (block) => {
        const title = (block.title || '').toLowerCase();
        return block?.fixedTime === true ||
            block?.blockType === 'important' ||
            block?.canSplit === true ||
            block?.blockType === 'work' ||
            title.includes('work');
    };
    const isCookBlock = (block) => {
        if (!block?.linkedBlockId) return false;
        if (block.isCookingBlock === true) return true;
        const title = (block.title || '').trim();
        return title.toLowerCase().startsWith('cook ');
    };
    const getRange = (block) => {
        if (!block?.time) return { start: NaN, end: NaN };
        const [startStr, endStr] = block.time.split('-');
        return {
            start: timeStrToMinutes(startStr),
            end: timeStrToMinutes(endStr)
        };
    };
    ...
    allBlocks.forEach(cookBlock => {
        if (!isCookBlock(cookBlock)) return;
        const mealBlock = allBlocks.find(block => block?.id && block.id === cookBlock.linkedBlockId);
        if (!mealBlock) return;
        const cookRange = getRange(cookBlock);
        const mealRange = getRange(mealBlock);
        if (isNaN(cookRange.start) || isNaN(cookRange.end) || isNaN(mealRange.start)) return;

        const cookDuration = cookRange.end - cookRange.start;
        const mealStart = mealRange.start;
        const requiredCookStart = mealStart - cookDuration;
        const requiredCookEnd = mealStart;
        let effectiveCookStart = Math.max(0, requiredCookStart);

        let movedBlocks = new Set();
        let totalShift = 0;
        let fullyCleared = true;

        const shiftBlockEarlier = (block, shiftBy, visited = new Set()) => {
            if (shiftBy <= 0) return true;
            if (visited.has(block)) return true;
            visited.add(block);
            const { start, end } = getRange(block);
            const newStart = start - shiftBy;
            const newEnd = end - shiftBy;
            if (newStart < 0) return false;
            for (const other of allBlocks) {
                if (other === block || other === cookBlock) continue;
                const otherRange = getRange(other);
                if (isNaN(otherRange.start) || isNaN(otherRange.end)) continue;
                if (overlaps(newStart, newEnd, otherRange.start, otherRange.end)) {
                    if (isFixedBlock(other)) return false;
                    const neededShift = otherRange.end - newStart;
                    if (!shiftBlockEarlier(other, neededShift, visited)) return false;
                }
            }
            block.time = `${formatMinutesToTime(newStart)}-${formatMinutesToTime(newEnd)}`;
        ...
        if (requiredCookStart < 0) {
            fullyCleared = false;
        }

        while (true) {
            const overlapper = allBlocks.find(block => {
                if (block === cookBlock) return false;
                const { start, end } = getRange(block);
                if (isNaN(start) || isNaN(end)) return false;
                return overlaps(effectiveCookStart, requiredCookEnd, start, end);
            });
            if (!overlapper) break;
            if (isFixedBlock(overlapper)) {
                fullyCleared = false;
                break;
            }
            const { end } = getRange(overlapper);
            const shiftBy = end - effectiveCookStart;
            if (!shiftBlockEarlier(overlapper, shiftBy)) {
                fullyCleared = false;
                break;
            }
        }

        if (mealStart < cookDuration) {
            fullyCleared = false;
        }

        cookBlock.time = `${formatMinutesToTime(effectiveCookStart)}-${formatMinutesToTime(requiredCookEnd)}`;
        const mealType = mealBlock.mealType || inferMealCategoryFromTitle(mealBlock.title) || 'meal';
        console.log(`🍳 Cook anchored: ${mealType} ${formatMinutesToTime(effectiveCookStart)}-${formatMinutesToTime(mealStart)}`);
        if (totalShift > 0) {
            console.log(`⏪ Pulled earlier to fit cook: shifted=${totalShift} minutes, movedBlocks=${movedBlocks.size}`);
        }
        if (!fullyCleared) {
            console.warn(`⚠️ Could not fully make room for cook-before-meal: ${mealBlock.title || 'Meal'} ${dayKey}`);
        }
    });
...
function finalizeDayBlocks(blocks, date, dayKey) {
    ...
    const cookAdjustedBlocks = enforceCookBeforeMealWithPull(groupedBlocks, dayKey);
    const sortedBlocks = cookAdjustedBlocks.slice().sort((a, b) => {
        const aStart = getBlockTimeRange(a).start;
        const bStart = getBlockTimeRange(b).start;
        return aStart - bStart;
    });
...
function findAvailableShoppingSlots(orderedDays, beforeDayIndex, minDurationMins = 90, minDaysToShow = 2) {
    ...
    const workSchedule = {};
    orderedDays.forEach(day => {
        if (!day?.workActual) return;
        const [start, end] = day.workActual.split('-');
        if (start && end) {
            workSchedule[day.date.getDay()] = { start, end, date: new Date(day.date) };
        }
    });
    const commuteSettings = getCommuteSettingsUniversal();
    ...
    const collectSlotsForDay = (dayIndex) => {
        ...
        const reservedWork = day.work || '';
        const actualWork = day.workActual || '';
        const prediction = buildPredictedDayBusyIntervals(
            date,
            (typeof isSetupWizardActive === 'function' && isSetupWizardActive()) ? {} : (day.hasFinalBlocks && day.blocks?.length ? { existingBlocks: day.blocks } : {}),
            scheduleData.defaultBlocks,
            workSchedule,
            commuteSettings,
            { cookingConfig: getCookingConfig() }
        );
        const mergedBusy = prediction.busyIntervals;
        const dayWindowStart = prediction.dayWindowStart;
        const dayWindowEnd = prediction.dayWindowEnd;
        console.log(`🛒 Shopping slots exclude lunch/cook chain time for ${dayName}`);
        console.log(`🗓️ ${dayName}: actualWork=${actualWork || 'none'} reservedWork=${reservedWork || 'none'}`);
        const gaps = [];
        ...
        console.log(`🗓️ ${dayName}: final free gaps=${gaps.map(gap => `${formatMinutesToTime(gap.start)}-${formatMinutesToTime(gap.end)}`).join(', ') || 'none'}`);

        const gapOverlapsSensitiveBlock = (gap) => {
            return prediction.blocks.some(block => {
                const title = (block.title || '').toLowerCase();
                const isCommuteOrPrep = title.includes('commute') || title.includes('prep');
                const isLunch = mealTypeOf(block) === 'lunch';
                const isCookLunch = block.isCookingBlock && (block.mealType === 'lunch' || title.includes('cook lunch'));
                if (!isCommuteOrPrep && !isLunch && !isCookLunch) return false;
                const { start, end } = getBlockTimeRange(block);
                if (isNaN(start) || isNaN(end)) return false;
                return gap.start < end && gap.end > start;
            });
        };

        gaps.forEach(gap => {
            if (gap.duration < minDurationMins) return;
            if (gapOverlapsSensitiveBlock(gap)) {
                console.log(`🧹 Removed gap due to lunch/cook/commute overlap: ${formatMinutesToTime(gap.start)}-${formatMinutesToTime(gap.end)}`);
                return;
            }

            let label = '';
            if (day.workActual) {
                const [actualStart, actualEnd] = day.workActual.split('-');
                const actualWorkStart = timeStrToMinutes(actualStart);
                const actualWorkEnd = timeStrToMinutes(actualEnd);
                if (!isNaN(actualWorkStart) && !isNaN(actualWorkEnd)) {
                    if (gap.start >= actualWorkEnd) {
                        label = 'After work';
                    } else if (gap.end <= actualWorkStart) {
                        label = 'Before work';
                    }
                }
            }
            ...
            console.log(`🕒 Free slot computed from FINAL blocks: start=${formatMinutesToTime(gap.start)} end=${formatMinutesToTime(gap.end)} (busyCount=${mergedBusy.length})`);
...
function showCookingConfigModal(isFirstTime = false) {
    ...
    if (setupWizardModal?.classList.contains('active')) {
        openSetupStep(3);
    } else {
        openSetupWizard();
        openSetupStep(3);
    ...
    if (isFirstTime) {
        console.log('⚙️ First time setup: opening cooking preferences step');
    }
...
function saveConfigFromForm(options = {}) {
...
// ================================================
// ENHANCED DEFAULT BLOCKS SYSTEM - V2.2.0
// ================================================
// Features:
// 1. Visual free time slots display
// 2. Common title dropdown with quick selection  
// 3. Block type selector (Normal, Commute, Important)
// 4. Important blocks can override existing blocks
// 5. Auto-duration calculation

// ================================================
// ENHANCED DEFAULT BLOCKS SYSTEM - V2.2.3
// ================================================
// Removed: Quick add button (now in separate module)
// Features: Free slots, quick titles, block types, prep/travel
...
const COMMON_TITLES = {
    routines: [
        { emoji: '🌅', title: 'Morning Routine', duration: 30 },
        { emoji: '🌙', title: 'Evening Routine', duration: 30 },
        { emoji: '😴', title: 'Sleep', duration: 480 },
        { emoji: '🧘', title: 'Meditation', duration: 15 },
        { emoji: '🚿', title: 'Shower', duration: 20 }
    ],
    work: [
        { emoji: '💼', title: 'Work', duration: 480 },
        { emoji: '👥', title: 'Meeting', duration: 60 },
        { emoji: '💻', title: 'Deep Work', duration: 120 },
        { emoji: '📧', title: 'Email', duration: 30 },
        { emoji: '📞', title: 'Phone Calls', duration: 30 }
    ],
    health: [
        { emoji: '🏋️', title: 'Gym', duration: 60 },
        { emoji: '🏃', title: 'Running', duration: 45 },
        { emoji: '🧘', title: 'Yoga', duration: 45 },
        { emoji: '🚴', title: 'Cycling', duration: 60 },
        { emoji: '⚽', title: 'Sports', duration: 90 }
    ],
    activities: [
        { emoji: '🧹', title: 'Cleaning', duration: 60 },
        { emoji: '🛒', title: 'Shopping', duration: 90 },
        { emoji: '📚', title: 'Reading', duration: 30 },
        { emoji: '🎮', title: 'Gaming', duration: 60 },
        { emoji: '🎬', title: 'Watch Movie', duration: 120 }
    ],
    travel: [
        { emoji: '🚗', title: 'Drive to Location', duration: 30 },
        { emoji: '🚶', title: 'Walk', duration: 20 },
        { emoji: '🚇', title: 'Public Transport', duration: 45 },
        { emoji: '✈️', title: 'Airport', duration: 180 }
    ],
    important: [
        { emoji: '💊', title: 'Take Medicine', duration: 5 },
        { emoji: '☎️', title: 'Important Call', duration: 15 },
        { emoji: '📋', title: 'Urgent Task', duration: 15 },
        { emoji: '🏥', title: 'Doctor Appointment', duration: 60 },
        { emoji: '💉', title: 'Medical Check', duration: 30 }
    ]
};

window.calculateFreeTimeSlots = function(selectedDays) {
    const freeSlots = {};
    
    selectedDays.forEach(day => { freeSlots[day] = []; });

    const dayEntries = Object.values(scheduleData.days || {});
    const setupStartInput = document.getElementById('setupWeekStartDateInput')?.value;
    const fallbackStartInput = document.getElementById('weekStartDateInput')?.value || document.getElementById('weekStartDate')?.value;
    const weekStartDate = parseFlexibleDate(setupStartInput || fallbackStartInput || '') || new Date();
    const dayIndexMap = {
        Sunday: 0,
        Monday: 1,
        Tuesday: 2,
    ...
    const orderedDays = selectedDays.map(dayName => {
        const offset = (dayIndexMap[dayName] ?? 0) - weekStartDate.getDay();
        const date = new Date(weekStartDate);
        date.setDate(weekStartDate.getDate() + offset);
        const existing = dayEntries.find(entry => entry?.name === dayName);
        return {
            name: dayName,
            date,
    ...
    selectedDays.forEach(dayName => {
        const day = orderedDays.find(entry => entry?.name === dayName);
        const prediction = buildPredictedDayBusyIntervals(
            day?.date,
            { existingBlocks: day?.blocks?.length ? day.blocks : null },
            scheduleData.defaultBlocks,
            workSchedule,
            commuteSettings,
            { cookingConfig: getCookingConfig() }
        ...
        const mergedBusy = prediction.busyIntervals;
        const gaps = [];
        let cursor = prediction.dayWindowStart;
        mergedBusy.forEach(interval => {
            if (interval.start > cursor) {
                gaps.push({
                    start: cursor,
        ...
        if (cursor < prediction.dayWindowEnd) {
            gaps.push({
                start: cursor,
                end: prediction.dayWindowEnd,
                duration: prediction.dayWindowEnd - cursor
            });
        }

        gaps.forEach(gap => {
            if (gap.duration < 5) return;
            const slot = {
            ...
            console.log(`🕒 Free slot computed from FINAL blocks: start=${slot.start} end=${slot.end} (busyCount=${mergedBusy.length})`);
            freeSlots[dayName].push(slot);
        });
    });
...
window.renderFreeTimeSlots = function() {
    const container = document.getElementById('freeTimeSlotsDisplay');
    if (!container) return;
    
    const selectedDays = Array.from(document.querySelectorAll('.default-day-checkbox:checked')).map(cb => cb.value);
    
    if (selectedDays.length === 0) {
        container.innerHTML = '<p style="color:#999;text-align:center;padding:20px;font-size:13px;">Select days to see available slots</p>';
        return;
    }
    
    const freeSlots = calculateFreeTimeSlots(selectedDays);
    let html = '<div style="max-height:300px;overflow-y:auto;padding:10px;">';
    
    selectedDays.forEach(day => {
        const slots = freeSlots[day];
        html += `<div style="margin-bottom:16px;"><div style="font-weight:700;color:#667eea;margin-bottom:8px;font-size:13px;">📅 ${day}</div>`;
        
        if (slots.length === 0) {
            html += '<div style="padding:8px;background:#fee2e2;border-radius:6px;font-size:12px;color:#b91c1c;">No free slots (07:00-23:00)</div>';
        } else {
            slots.forEach(slot => {
                const h = Math.floor(slot.duration / 60);
                const m = slot.duration % 60;
                const dur = h > 0 ? `${h}h ${m}m` : `${m}m`;
                html += `<div onclick="quickFillTimeSlot('${slot.start}','${slot.end}')" style="padding:10px;background:#f0fdf4;border:1px solid #86efac;border-radius:6px;margin-bottom:6px;cursor:pointer;transition:all 0.2s;" onmouseover="this.style.background='#dcfce7';this.style.borderColor='#4ade80';" onmouseout="this.style.background='#f0fdf4';this.style.borderColor='#86efac';"><div style="display:flex;justify-content:space-between;align-items:center;"><span style="font-weight:600;color:#065f46;font-size:12px;">⏰ ${slot.start} - ${slot.end}</span><span style="background:#4ade80;color:white;padding:2px 8px;border-radius:12px;font-size:10px;font-weight:700;">${dur}</span></div><div style="font-size:10px;color:#16a34a;margin-top:4px;">${slot.description}</div></div>`;
            });
        }
        html += '</div>';
    });
...
window.quickFillTimeSlot = function(start, end) {
    document.getElementById('defaultStartTime').value = start;
    document.getElementById('defaultEndTime').value = end;
    ['defaultStartTime', 'defaultEndTime'].forEach(id => {
...
window.switchTitleCategory = function(category) {
    document.querySelectorAll('.title-category-btn').forEach(btn => {
    ...
    // AUTO-SET BLOCK TYPE: Important category = important blocks
    const autoBlockTypeInput = document.getElementById('autoBlockType');
    if (autoBlockTypeInput) {
        if (category === 'important') {
            autoBlockTypeInput.value = 'important';
            console.log('✅ Auto-set block type to: important');
        } else {
            autoBlockTypeInput.value = 'normal';
        }
    }
    
    renderTitleOptions(category);
};

function renderTitleOptions(category) {
    const container = document.getElementById('titleOptionsGrid');
    if (!container) return;
    
    const titles = COMMON_TITLES[category] || [];
    let html = '';
    titles.forEach(item => {
        html += `<button type="button" onclick="selectTitleOption('${item.emoji}','${item.title.replace(/'/g, "\\'")}',${item.duration})" style="padding:8px;background:white;border:1px solid #d1d5db;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;text-align:center;transition:all 0.2s;" onmouseover="this.style.background='#f0fdf4';this.style.borderColor='#86efac';" onmouseout="this.style.background='white';this.style.borderColor='#d1d5db';"><div style="font-size:18px;margin-bottom:2px;">${item.emoji}</div><div style="color:#374151;font-size:10px;line-height:1.2;">${item.title}</div></button>`;
    });
    container.innerHTML = html;
}

window.selectTitleOption = function(emoji, title, duration) {
    const fullTitle = `${emoji} ${title}`;
    const titleInput = document.getElementById('defaultTitle');
    titleInput.value = fullTitle;
    
    // Dispatch input event to trigger all listeners
    ...
    if (startInput.value && !endInput.value) {
        const startMins = timeStrToMinutes(startInput.value);
        endInput.value = formatMinutesToTime(startMins + duration);
    }
    
    titleInput.style.background = '#dcfce7';
    setTimeout(() => titleInput.style.background = '', 500);
    showToast(`✅ ${fullTitle}`);
};

window.updateBlockTypeDisplay = function() {
    const selected = document.querySelector('input[name="blockType"]:checked')?.value || 'normal';
    const desc = document.getElementById('blockTypeDescription');
    if (!desc) return;
    
    const info = {
        normal: { text: '<strong>Normal</strong> - Standard block that won\'t overlap', color: '#f0f9ff', textColor: '#0369a1' },
        important: { text: '<strong>⚠️ Important</strong> - Splits existing blocks and continues after', color: '#fee2e2', textColor: '#991b1b' }
    };
    
    desc.innerHTML = info[selected].text;
    desc.style.background = info[selected].color;
    desc.style.color = info[selected].textColor;
};

window.togglePrepTravelOptions = function() {
    const checkbox = document.getElementById('addPrepTravelToggle');
    const options = document.getElementById('prepTravelOptions');
    if (options) {
        options.style.display = checkbox.checked ? 'block' : 'none';
    }
};

function injectEnhanced() {
    const modal = document.getElementById('addEditDefaultModal');
    if (!modal || document.getElementById('enhancedInjected')) return;
    ...
    marker.type = 'hidden';
    marker.id = 'enhancedInjected';
    form.appendChild(marker);
    
    const daysSection = form.querySelector('.form-group');
    if (!daysSection) return;
    
    // Available Time Slots section REMOVED - not needed in default blocks
    
    const timeSection = Array.from(form.querySelectorAll('.form-group')).find(el => el.textContent.includes('Time:'));
    if (timeSection) {
        // Add hidden input to track if block is important (auto-set by category selection)
        timeSection.insertAdjacentHTML('afterend', `
            <input type="hidden" id="autoBlockType" value="normal">
            
            <div style="margin-bottom:20px;"><div style="background:#e0f2fe;border:2px solid #38bdf8;padding:12px;border-radius:8px;"><label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-bottom:10px;"><input type="checkbox" id="addPrepTravelToggle" onchange="togglePrepTravelOptions()" style="width:18px;height:18px;"><span style="font-weight:600;color:#0369a1;font-size:13px;">🚗 Add Preparation & Travel Time</span></label><div id="prepTravelOptions" style="display:none;padding-left:26px;"><div style="margin-bottom:8px;"><label style="font-size:12px;color:#0c4a6e;display:block;margin-bottom:4px;">Preparation time (before):</label><input type="number" id="prepDuration" min="0" max="120" value="15" style="width:80px;padding:6px;border:1px solid #38bdf8;border-radius:4px;font-size:12px;"> minutes</div><div><label style="font-size:12px;color:#0c4a6e;display:block;margin-bottom:4px;">Travel time (each way):</label><input type="number" id="travelDuration" min="0" max="180" value="30" style="width:80px;padding:6px;border:1px solid #38bdf8;border-radius:4px;font-size:12px;"> minutes</div></div></div></div>
        `);
    }
    
    const titleSection = Array.from(form.querySelectorAll('.form-group')).find(el => el.textContent.includes('Title:'));
    if (titleSection) {
        // Insert Quick Titles only (medicine form moved to separate button)
        titleSection.insertAdjacentHTML('afterend', `
            <div style="margin-bottom:20px;">
                <div style="background:#fef3c7;border:2px solid #fbbf24;padding:12px;border-radius:8px;">
                    <div style="font-weight:600;color:#78350f;margin-bottom:8px;font-size:13px;">⚡ Quick Titles</div>
                    <div style="display:flex;gap:4px;margin-bottom:10px;overflow-x:auto;">
                        <button type="button" onclick="switchTitleCategory('routines')" class="title-category-btn" data-category="routines" style="padding:5px 10px;background:#667eea;color:white;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">🌅</button>
                        <button type="button" onclick="switchTitleCategory('work')" class="title-category-btn" data-category="work" style="padding:5px 10px;background:#e0e0e0;color:#333;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">💼</button>
                        <button type="button" onclick="switchTitleCategory('health')" class="title-category-btn" data-category="health" style="padding:5px 10px;background:#e0e0e0;color:#333;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">🏋️</button>
                        <button type="button" onclick="switchTitleCategory('activities')" class="title-category-btn" data-category="activities" style="padding:5px 10px;background:#e0e0e0;color:#333;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">🎮</button>
                        <button type="button" onclick="switchTitleCategory('travel')" class="title-category-btn" data-category="travel" style="padding:5px 10px;background:#e0e0e0;color:#333;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">🚗</button>
                        <button type="button" onclick="switchTitleCategory('important')" class="title-category-btn" data-category="important" style="padding:5px 10px;background:#e0e0e0;color:#333;border:none;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">⚠️</button>
                    </div>
                    <div id="titleOptionsGrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:6px;max-height:200px;overflow-y:auto;"></div>
                </div>
            </div>
        `);
        renderTitleOptions('routines');
    }
}

const origOpen = window.openAddDefaultModal;
window.openAddDefaultModal = function() {
...
window.ensureMedicineButton = ensureMedicineButton;
...
window.openMedicineScheduleModal = function() {
    let modal = document.getElementById('medicineScheduleModal');
    
    if (!modal) {
        // Create modal
        modal = document.createElement('div');
        modal.id = 'medicineScheduleModal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content" style="max-width:600px;">
                <h2 style="margin-top:0;color:#667eea;">💊 Medicine Schedule Auto-Fill</h2>
                <div style="margin-bottom:15px;">
                    <label style="display:block;font-weight:600;margin-bottom:5px;">Medicine Name:</label>
                    <input type="text" id="medicineName" placeholder="e.g., Aspirin" style="width:100%;padding:10px;border:2px solid #ddd;border-radius:6px;">
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;margin-bottom:15px;">
                    <div>
                        <label style="display:block;font-weight:600;margin-bottom:5px;">Times per day:</label>
                        <input type="number" id="medicineTimesPerDay" min="1" max="10" value="2" onchange="updateMedicineTimeInputs()" style="width:100%;padding:10px;border:2px solid #ddd;border-radius:6px;">
                    </div>
                    <div>
                        <label style="display:block;font-weight:600;margin-bottom:5px;">Duration (minutes):</label>
                        <input type="number" id="medicineDuration" min="1" max="60" value="2" style="width:100%;padding:10px;border:2px solid #ddd;border-radius:6px;">
                    </div>
                </div>
                <div id="medicineTimesInputs" style="margin-bottom:15px;"></div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;margin-bottom:20px;">
                    <div>
                        <label style="display:block;font-weight:600;margin-bottom:5px;">Start Date:</label>
                        <input type="date" id="medicineStartDate" style="width:100%;padding:10px;border:2px solid #ddd;border-radius:6px;">
                    </div>
                    <div>
                        <label style="display:block;font-weight:600;margin-bottom:5px;">End Date:</label>
                        <input type="date" id="medicineEndDate" style="width:100%;padding:10px;border:2px solid #ddd;border-radius:6px;">
                    </div>
                </div>
                <div style="display:flex;gap:10px;">
                    <button onclick="generateMedicineDefaultBlocks()" style="flex:1;padding:12px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;border:none;border-radius:8px;font-weight:700;cursor:pointer;">
                        ✨ Generate Schedule
                    </button>
                    <button onclick="closeMedicineScheduleModal()" style="padding:12px 20px;background:#e5e7eb;color:#374151;border:none;border-radius:8px;font-weight:700;cursor:pointer;">
                        Cancel
                    </button>
                </div>
            </div>
    ...
    updateMedicineTimeInputs();
    modal.classList.add('active');
};

window.closeMedicineScheduleModal = function() {
    const modal = document.getElementById('medicineScheduleModal');
    if (modal) modal.classList.remove('active');
};

// Update medicine time inputs
window.updateMedicineTimeInputs = function() {
    const timesPerDay = parseInt(document.getElementById('medicineTimesPerDay')?.value || 2);
    const container = document.getElementById('medicineTimesInputs');
    if (!container) return;
    ...
    for (let i = 1; i <= timesPerDay; i++) {
        ...
        const timeDiv = document.createElement('div');
        timeDiv.style.cssText = 'margin-bottom:10px;';
        timeDiv.innerHTML = `
            <label style="display:block;font-weight:600;margin-bottom:5px;">Dose ${i} - Time:</label>
            <input type="time" class="medicine-time-input" value="${defaultTime}" style="width:100%;padding:10px;border:2px solid #ddd;border-radius:6px;">
        `;
        container.appendChild(timeDiv);
    }
};

const origEdit = window.editDefaultBlock;
window.editDefaultBlock = function(index) {
    if (origEdit) origEdit(index);
    setTimeout(() => {
        injectEnhanced();
        const block = scheduleData.defaultBlocks[index];
        if (block?.blockType) {
            const autoBlockTypeInput = document.getElementById('autoBlockType');
            if (autoBlockTypeInput) {
                autoBlockTypeInput.value = block.blockType;
            }
        }
        if (block?.hasPrepTravel) {
            document.getElementById('addPrepTravelToggle').checked = true;
            togglePrepTravelOptions();
            if (block.prepDuration) document.getElementById('prepDuration').value = block.prepDuration;
            if (block.travelDuration) document.getElementById('travelDuration').value = block.travelDuration;
        }
    }, 50);
};

const origSave = window.saveDefaultBlock;
window.saveDefaultBlock = function(event) {
    const blockType = document.getElementById('autoBlockType')?.value || 'normal';
    const hasPrepTravel = document.getElementById('addPrepTravelToggle')?.checked || false;
    const prepDuration = hasPrepTravel ? parseInt(document.getElementById('prepDuration')?.value || 15) : 0;
    const travelDuration = hasPrepTravel ? parseInt(document.getElementById('travelDuration')?.value || 30) : 0;
    const index = parseInt(document.getElementById('defaultBlockIndex').value);
    
    console.log('💾 Saving default block with type:', blockType);
    
    const result = origSave(event);
    
    const targetIndex = index >= 0 ? index : (scheduleData.defaultBlocks.length - 1);
    const block = scheduleData.defaultBlocks[targetIndex];
    
    if (block) {
        if (blockType !== 'normal') {
            block.blockType = blockType;
            if (blockType === 'important') block.canSplit = true;
        }
        
        if (hasPrepTravel) {
            block.hasPrepTravel = true;
            block.prepDuration = prepDuration;
            block.travelDuration = travelDuration;
        }
...
console.log('✅ Enhanced Default Blocks - V2.2.3');
...
// ================================================
// ================================================
// FINAL WORKING FIX - V2.3.1
// ================================================
// Fixed: Button added to correct VISIBLE day element
// Fixed: Free slots 07:00-23:00 bug (last line in image)

(function() {
'use strict';

// ==============================================
// IMPORTANT BLOCK SPLITTING
// ==============================================

const originalCopyDefaults = window.copyDefaultsToDay;
...
function reapplySplittingToAllDays() {
    Object.keys(scheduleData.days).forEach(dayKey => {
        ...
        day.blocks = finalizeDayBlocks(day.blocks, dayDate, dayKey);
    });
    saveToLocalStorage();
}
...
if (originalCopyDefaults) {
    window.copyDefaultsToDay = function(dayKey, dayName) {
        originalCopyDefaults.call(this, dayKey, dayName);
...
const originalSaveDefaultBlock = window.saveDefaultBlock;
window.saveDefaultBlock = function(event) {
    if (event) event.preventDefault();
    
    const blockType = document.querySelector('input[name="blockType"]:checked')?.value || 'normal';
    const hasPrepTravel = document.getElementById('addPrepTravelToggle')?.checked || false;
    const prepDuration = hasPrepTravel ? parseInt(document.getElementById('prepDuration')?.value || 15) : 0;
    const travelDuration = hasPrepTravel ? parseInt(document.getElementById('travelDuration')?.value || 30) : 0;
    ...
    window._pendingBlockMetadata = {
        blockType: blockType,
        canSplit: blockType === 'important',
        hasPrepTravel: hasPrepTravel,
        prepDuration: prepDuration,
        travelDuration: travelDuration
    };
    
    const result = originalSaveDefaultBlock.call(this, event);
    delete window._pendingBlockMetadata;
    return result;
};

const originalValidate = window.validateDefaultBlock;
function validateDefaultBlockOverride(newBlock, editingIndex) {
    const pendingMeta = window._pendingBlockMetadata;
    const isImportant = pendingMeta?.canSplit || newBlock.canSplit || newBlock.blockType === 'important';
    
    if (isImportant) {
        console.log('✅ Important block - allowing overlaps');
        const timeRange = getBlockTimeRange(newBlock);
        if (isNaN(timeRange.start) || isNaN(timeRange.end)) {
            alert('⚠️ Invalid times');
            return false;
        }
        if (timeRange.end <= timeRange.start) {
            alert('⚠️ End must be after start');
            return false;
        }
        if (pendingMeta) {
            Object.assign(newBlock, pendingMeta);
        }
        return true;
    }
    
    return originalValidate ? originalValidate(newBlock, editingIndex) : true;
}
window.validateDefaultBlock = validateDefaultBlockOverride;

// ==============================================
// FREE SLOTS - NO MORE 07:00-23:00 BUG
// ==============================================

window.quickFillDayBlockTime = function(start, end) {
    document.getElementById('startTime').value = start;
    document.getElementById('endTime').value = end;
    document.getElementById('blockTime').value = `${start}-${end}`;
...
function calculateDayFreeSlots(dayKey) {
    const day = scheduleData.days?.[dayKey];
...
function injectFreeSlotsIntoEditModal(dayKey) {
    const modal = document.getElementById('editModal');
    if (!modal) return;
    
    const existing = modal.querySelector('#dayFreeSlotsDisplay');
    if (existing) existing.remove();
    
    const freeSlots = calculateDayFreeSlots(dayKey);
    const timeInput = modal.querySelector('#blockTime, #startTime');
    if (!timeInput) return;
    
    const timeSection = timeInput.closest('.form-group') || timeInput.parentElement;
    if (!timeSection) return;
    
    const div = document.createElement('div');
    div.id = 'dayFreeSlotsDisplay';
    div.innerHTML = `
        <div style="background:#f0fdf4;border:2px solid #86efac;padding:12px;border-radius:8px;margin-bottom:15px;">
            <div style="font-weight:700;color:#065f46;margin-bottom:8px;font-size:13px;">📊 Available Time Slots</div>
            ${freeSlots.length === 0 ? 
                '<div style="padding:8px;background:#fee2e2;border-radius:6px;font-size:12px;color:#b91c1c;">No free slots</div>' :
                freeSlots.map(slot => {
                    const h = Math.floor(slot.duration / 60);
                    const m = slot.duration % 60;
                    const dur = h > 0 ? `${h}h ${m}m` : `${m}m`;
                    return `<div onclick="quickFillDayBlockTime('${slot.start}','${slot.end}')" style="padding:10px;background:#f0fdf4;border:1px solid #86efac;border-radius:6px;margin-bottom:6px;cursor:pointer;" onmouseover="this.style.background='#dcfce7';" onmouseout="this.style.background='#f0fdf4';"><div style="display:flex;justify-content:space-between;"><span style="font-weight:600;color:#065f46;font-size:12px;">⏰ ${slot.start} - ${slot.end}</span><span style="background:#4ade80;color:white;padding:2px 8px;border-radius:12px;font-size:10px;font-weight:700;">${dur}</span></div><div style="font-size:10px;color:#16a34a;margin-top:4px;">${slot.description}</div></div>`;
                }).join('')
            }
        </div>
    `;
    
    timeSection.insertAdjacentElement('beforebegin', div);
}

window.openQuickAddBlock = function(dayKey) {
    // If dayKey provided, use it directly
    if (!dayKey) {
        let activeTab = document.querySelector('.day-tab.active');
        if (!activeTab) {
            const headers = document.querySelectorAll('h2, h3');
            for (let h of headers) {
                const match = h.textContent.match(/(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)/);
                if (match) {
                    activeTab = { dataset: { day: match[1] } };
                    break;
                }
            }
        }
        
        if (!activeTab?.dataset?.day) {
            showToast('⚠️ No active day');
            return;
        }
        
        const dayName = activeTab.dataset.day;
        const dayKeys = Object.keys(scheduleData.days || {});
        dayKey = dayKeys.find(k => k.endsWith(`-${dayName}`)) ||
                     dayKeys.find(k => k.toLowerCase().includes(dayName.toLowerCase()));
    }
    
    if (!dayKey) {
        showToast(`⚠️ Day not found`);
        return;
    }
    
    const lastIndex = (scheduleData.days[dayKey]?.blocks?.length || 1) - 1;
    if (typeof addNewBlock === 'function') {
        addNewBlock(dayKey, lastIndex);
        setTimeout(() => injectFreeSlotsIntoEditModal(dayKey), 100);
    }
};

console.log('✅ Clean Fix V2.3.2 - Button in HTML, no flickering!');
