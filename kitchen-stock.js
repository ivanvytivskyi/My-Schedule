        ...
        <div style="max-width: 900px; margin: 0 auto;">
            ...
            <div style="background: #fff7ed; border: 2px dashed #fdba74; padding: 16px; border-radius: 12px; margin-bottom: 18px;">
                ...
                <form onsubmit="addNewMeasurement(event)" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; align-items: end;">
                    <div>
                        <label style="display: block; font-weight: 600; color: #7c2d12; margin-bottom: 4px;">Name (what you see)</label>
                        <input id="newMeasureLabel" type="text" required placeholder="e.g., Cup of Quinoa" style="width: 100%; padding: 10px; border: 2px solid #fdba74; border-radius: 8px; font-size: 14px;">
                    </div>
                    <div>
                ...
                <p style="margin: 10px 0 0 0; font-size: 12px; color: #9a3412;">We auto-generate a safe key from the name for recipe units (e.g., “2 cup_quinoa”). Values are stored in base units so shopping and cooking stay accurate.</p>
            </div>
...
function addNewMeasurement(event) {
    ...
    if (!labelInput || !valueInput || !unitInput) return;
    ...
    if (!sanitizedKey) {
        alert('Please use letters and numbers for the name');
        return;
    }
    ...
    if (customMeasurements[sanitizedKey]) {
        alert('A custom measurement with this name already exists.');
        return;
    }
