(() => {
    ...
    function isLinkedToFixedTitle(block, allBlocks) {
        if (!block || !Array.isArray(allBlocks)) return false;
        const baseId = block.linkedBlockId || block.id;
        if (!baseId) return false;
        const chainBlocks = allBlocks.filter(item =>
            item?.id === baseId || item?.linkedBlockId === baseId
        );
        return chainBlocks.some(item =>
            item.fixedTime === true || isStudyTitle(item) || allowedDuringWork(item)
        );
    }

    function isFixedOverlapBlock(block, allBlocks) {
        return block?.fixedTime === true ||
            isStudyTitle(block) ||
            isPackingRoutineTitle(block) ||
            isMealTitle(block) ||
            isLinkedToFixedTitle(block, allBlocks);
    }
    ...
    function getLinkedGroupKey(block, allBlocks) {
        if (block?.linkedBlockId) return `linked:${block.linkedBlockId}`;
        const hasLinkedChildren = allBlocks.some(item => item?.linkedBlockId === block?.id);
        if (hasLinkedChildren && block?.id) return `linked:${block.id}`;
        return `single:${block?.id || block?.title || Math.random()}`;
    }

    function getLinkedGroupBlocks(block, allBlocks) {
        const baseId = block?.linkedBlockId || block?.id;
        if (!baseId) return [block];
        const chainBlocks = allBlocks.filter(item =>
            item?.id === baseId || item?.linkedBlockId === baseId
        );
        return chainBlocks.length ? chainBlocks : [block];
    }
    ...
    function resolveWorkOverlaps(dayIndex, dayData, blocks, workRange) {
        return new Promise((resolve, reject) => {
            ...
            const overlapping = blocks.filter(block => {
                ...
                if (isFixedOverlapBlock(block, blocks)) return false;
    ...
    function resolveOverlapConflicts(dayData, blocks, conflictBlocks, summaryText = '') {
        return new Promise((resolve, reject) => {
            if (!Array.isArray(blocks) || !Array.isArray(conflictBlocks) || conflictBlocks.length === 0) {
                resolve(blocks || []);
                return;
            }
            ...
            overlapResolverState = {
                dayIndex: 0,
                dayData,
                overlappingBlocks: conflictBlocks,
                workRange: null,
                allBlocks: blocks,
                resolveCallback: resolve,
                rejectCallback: reject,
    ...
    function prepareOverlapTaskStates() {
        ...
        overlapResolverState.overlappingBlocks.forEach(block => {
            const key = getLinkedGroupKey(block, overlapResolverState.allBlocks);
            if (!grouped.has(key)) grouped.set(key, []);
            const chainBlocks = getLinkedGroupBlocks(block, overlapResolverState.allBlocks);
            grouped.get(key).push(...chainBlocks);
        });

        overlapResolverState.taskStates = Array.from(grouped.entries()).map(([groupKey, groupBlocks]) => {
            const blocks = Array.from(new Set(groupBlocks));
            const duration = blocks.reduce((sum, block) => {
                const blockDuration = typeof getBlockDurationMinutes === 'function'
                    ? getBlockDurationMinutes(block)
                    : 0;
                return sum + blockDuration;
            }, 0);
            const starts = blocks.map(block => {
                const { start } = typeof getBlockTimeRange === 'function' ? getBlockTimeRange(block) : { start: NaN };
                return start;
            }).filter(value => !isNaN(value));
            const groupStart = starts.length ? Math.min(...starts) : 0;
            const offsets = blocks.map(block => {
                const { start } = typeof getBlockTimeRange === 'function'
                    ? getBlockTimeRange(block)
                    : { start: NaN };
                return {
                    block,
                    offsetStart: isNaN(start) ? 0 : start - groupStart,
                    duration: typeof getBlockDurationMinutes === 'function' ? getBlockDurationMinutes(block) : 0
                };
            });
            ...
            const primaryBlock = blocks.find(block => !block.isCookingBlock) || blocks[0];
            return {
                block: primaryBlock,
                blocks,
                groupKey,
    ...
    function showOverlapList() {
        ...
        if (summary) {
            summary.textContent = overlapResolverState.summaryText
                ? overlapResolverState.summaryText
                : `Some blocks overlap your work hours for ${dayName}. Choose what you’d like to do.`;
        }
    ...
    function renderOverlapTasks() {
        ...
        overlapResolverState.taskStates.forEach((state, index) => {
            ...
            const duration = state.duration ?? (typeof getBlockDurationMinutes === 'function'
                ? getBlockDurationMinutes(block)
                : 0);
            const card = document.createElement('div');
            card.className = 'overlap-task-card';
            card.dataset.index = index;
            const groupLabel = state.blocks && state.blocks.length > 1
                ? ` (+${state.blocks.length - 1} linked)`
                : '';
            ...
            header.innerHTML = `
                <label style="display:flex; align-items:center; gap:8px; flex:1; cursor:pointer;">
                    ...
                    <span style="font-weight:700; color:#111827;">${block.title || 'Untitled'}${groupLabel}</span>
                </label>
            ...
            if (state.slots.length > 0) {
                ...
                `;
            } else if (state.blocks && state.blocks.length > 1) {
                moveControls.innerHTML = `
                    <div class="slot-preview-text" style="color:#1f2937;">No slot fits this linked chain.</div>
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        <button type="button" class="btn btn-secondary" data-action="remove-one" data-index="${index}" style="background:#fee2e2; color:#991b1b;">Remove this chain</button>
                    </div>
    ...
    function confirmSplitSelection(index) {
        ...
        if (preview) {
            const duration = state.duration ?? getBlockDurationMinutes(state.block);
    ...
    function removeSelectedOverlaps() {
        ...
        overlapResolverState.taskStates.filter(t => t.selected).forEach(state => {
            (state.blocks || [state.block]).forEach(block => toRemove.add(block));
        });
    ...
    function applyMoveSelections() {
        ...
        overlapResolverState.taskStates.forEach(state => {
            ...
            if (state.slots.length > 0) {
                ...
                if (state.blocks && state.blocks.length > 1) {
                    state.offsets.forEach(offset => {
                        const blockStart = start + offset.offsetStart;
                        const blockEnd = blockStart + offset.duration;
                        const updatedBlock = { ...offset.block, time: `${formatMinutesToTime(blockStart)}-${formatMinutesToTime(blockEnd)}` };
                        if (offset.block.startDateTime && overlapResolverState.dayData?.date) {
                            const date = new Date(overlapResolverState.dayData.date);
                            updatedBlock.startDateTime = createDateTimeFromTimeStr(date, formatMinutesToTime(blockStart)).toISOString();
                            updatedBlock.endDateTime = createDateTimeFromTimeStr(date, formatMinutesToTime(blockEnd)).toISOString();
                        }
    ...
    window.resolveOverlapConflicts = resolveOverlapConflicts;
