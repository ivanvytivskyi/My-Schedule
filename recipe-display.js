...
function generateShoppingListFromRecipes() {
    ...
    allRecipes.forEach(recipe => {
        (recipe.display?.ingredients || []).forEach(text => {
            ...
            if (!normalized || seenIngredients.has(normalized)) return;
            seenIngredients.add(normalized);
            ingredientList.push(text.trim());
        });
    });
    
    if (ingredientList.length === 0) {
        alert('No ingredients found in the selected recipes.');
        return;
    }
    
    const rows = ingredientList.map(name => `
        <tr>
    ...
    const html = `
        <div class="shopping-list-block" data-shop="Ingredients" style="margin-bottom: 24px; page-break-inside: avoid; border: 2px solid #e5e7eb; border-radius: 10px; overflow: hidden;">
            ...
            <table style="width: 100%; border-collapse: collapse;" data-shop="Ingredients">
                <thead>
