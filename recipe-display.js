...
function openRecipeModal(recipeId, scaledRecipe = null) {
    ...
    // Use scaled recipe if provided, otherwise use original
    const recipe = scaledRecipe || originalRecipe;
    ...
    if (scaledRecipe) {
        console.log('🔄 Using SCALED recipe (for', scaledRecipe.serves, 'people)');
    }
    ...
    if (recipe.display && recipe.display.instructions && Array.isArray(recipe.display.instructions)) {
        instructionsHTML = recipe.display.instructions.map(step => `<li>${step}</li>`).join('');
    } else if (originalRecipe.display && originalRecipe.display.instructions && Array.isArray(originalRecipe.display.instructions)) {
        // Fallback to original recipe instructions (they don't scale)
        instructionsHTML = originalRecipe.display.instructions.map(step => `<li>${step}</li>`).join('');
    }
    
    // Build tips section
    let tipsHTML = '';
    const displayData = recipe.display || originalRecipe.display;
    if (displayData && displayData.tips && Array.isArray(displayData.tips) && displayData.tips.length > 0) {
        tipsHTML = `
            ...
            <div style="background: #fff8e1; padding: 15px; border-radius: 8px; border-left: 4px solid #ffc107; margin-bottom: 20px;">
                ${displayData.tips.map(tip => `<p style="margin: 0 0 8px 0; color: #5d4037; font-size: 14px; line-height: 1.6;">${tip}</p>`).join('')}
            </div>
        `;
    }
    
    // Add scaling indicator if recipe is scaled
    let scalingBadge = '';
    if (scaledRecipe && scaledRecipe.isScaled) {
        scalingBadge = `
            <div style="background: #fff3cd; border: 2px solid #ffc107; padding: 12px 16px; border-radius: 8px; margin-bottom: 15px; display: flex; align-items: center; gap: 10px;">
                <span style="font-size: 24px;">⚖️</span>
                <div>
                    <strong style="color: #856404; display: block; font-size: 14px;">Recipe scaled for ${scaledRecipe.serves} person${scaledRecipe.serves > 1 ? 's' : ''}</strong>
                    <span style="color: #856404; font-size: 13px;">Original recipe serves ${scaledRecipe.originalServes}</span>
                </div>
            </div>
        `;
    }
    ...
    modal.innerHTML = `
        <div style="background: white; border-radius: 12px; max-width: 900px; width: 100%; margin: 20px auto; position: relative; padding: 30px;">
            ...
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 12px 12px 0 0; color: white; text-align: center; position: relative; margin: -30px -30px 20px -30px;">
                ...
                <div style="font-size: 64px; margin-bottom: 10px;">${(recipe.display || originalRecipe.display).emoji}</div>
                <h2 style="margin: 0 0 10px 0; font-size: 28px;">${recipe.name || originalRecipe.name}</h2>
                <p style="margin: 0; color: rgba(255,255,255,0.95); font-size: 16px; line-height: 1.5;">${(recipe.display || originalRecipe.display).description}</p>
                <div style="display: flex; gap: 15px; justify-content: center; margin-top: 15px;">
                    <span style="background: rgba(255,255,255,0.2); padding: 8px 15px; border-radius: 8px;">⏱️ ${recipe.prepTime || originalRecipe.prepTime || 'N/A'} + ${recipe.cookTime || originalRecipe.cookTime || 'N/A'}</span>
                    <span style="background: rgba(255,255,255,0.2); padding: 8px 15px; border-radius: 8px;">🍽️ Serves ${recipe.serves || originalRecipe.serves || 'N/A'}</span>
                </div>
            </div>
            ...
            <div style="padding: 30px;">
                ...
                <!-- Video -->
                ${(recipe.display || originalRecipe.display).video ? `
                    <a href="${(recipe.display || originalRecipe.display).video}" target="_blank" style="display: block; text-decoration: none; background: linear-gradient(135deg, #4caf50 0%, #45a049 100%); color: white; padding: 14px 20px; border-radius: 8px; text-align: center; font-weight: 600; font-size: 15px; margin-bottom: 15px; transition: all 0.2s;">
                ...
                <!-- Nutrition -->
                ${(recipe.display || originalRecipe.display).nutrition ? `
                ...
                <div style="display: flex; gap: 10px; margin-top: 20px; justify-content: flex-end;">
                    ${isCustom ? `
                        <button onclick="closeRecipeModal(); openCustomRecipeModal('${originalRecipe.id}');" style="padding: 12px 24px; background: #667eea; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">✏️ Edit</button>
                        <button onclick="deleteFromRecipeModal('${originalRecipe.id}')" style="padding: 12px 24px; background: #f44336; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">🗑️ Delete</button>
                    ` : ''}
...
console.log('✅ Recipe Display UI loaded - V2.1.1 (Scaled Recipe Support)!');
