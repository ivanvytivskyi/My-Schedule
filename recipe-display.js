            ...
            const availableUnits = match.unlimited
                ...
                : typeof match.qtyUnits === 'number'
                    ...
                    : match.qtyAvailablePacks
                        ? match.qtyAvailablePacks * (item.packSize || 1)
                        : 0;
            if (availableUnits === Infinity) {
                item.totalUnits = 0;
            } else {
                item.totalUnits = Math.max(0, item.totalUnits - availableUnits);
            }
        }
        
        item.qtyNeeded = item.totalUnits <= 0
            ? 0
            : Math.max(1, Math.ceil(item.totalUnits / (item.packSize || 1)));
    });
...
function loadHomeInventoryItems() {
    try {
        ...
        return parsed.map(item => {
            const unlimited = Boolean(item.unlimited) || (item.shop === 'Tap' && (item.qtyUnits === null || item.qtyUnits === undefined || item.qtyUnits === 'unlimited'));
            const qtyUnitsRaw = item.qtyUnits ?? item.qtyAvailable ?? 0;
