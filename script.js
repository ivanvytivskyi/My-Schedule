...
function packMorningBlocksBeforeWork(blocks, workStartTime) {
    ...
    // Trim earliest (last in priority) if total exceeds available
    let totalDuration = descriptors.reduce((sum, d) => sum + d.duration, 0);
    if (totalDuration > available && descriptors.length > 0) {
        const overflow = totalDuration - available;
        const lastIdx = descriptors.length - 1; // should be sleep after sort
        descriptors[lastIdx].duration = Math.max(0, descriptors[lastIdx].duration - overflow);
        totalDuration = descriptors.reduce((sum, d) => sum + d.duration, 0);
    }
