...
function buildPredictedDayBusyIntervals(dayDate, config = {}, defaultBlocks = [], workSchedule = {}, commuteSettings = {}, mealSettings = {}) {
    ...
    if (!existingBlocks) {
        ...
        if (work && work.start && work.end) {
            const commuteDuration = Number.isFinite(commuteSettings.commuteDuration) ? commuteSettings.commuteDuration : 0;
            const commutePrepDuration = Number.isFinite(commuteSettings.commutePrepDuration) ? commuteSettings.commutePrepDuration : 0;
            const addCommute = commuteSettings.addCommute === true;
            const addCommutePrep = commuteSettings.addCommutePrep === true;
...
function findAvailableShoppingSlots(orderedDays, beforeDayIndex, minDurationMins = 90, minDaysToShow = 2) {
    ...
    const commuteSettings = {
        addCommute: document.getElementById('addCommuteNew')?.checked || document.getElementById('addCommute')?.checked,
        addCommutePrep: document.getElementById('addCommutePrepNew')?.checked || false,
        commuteDuration: parseInt(document.getElementById('commuteDurationNew')?.value || document.getElementById('commuteDuration')?.value || '0', 10),
        commutePrepDuration: parseInt(document.getElementById('commutePrepDurationNew')?.value || '0', 10)
    };
