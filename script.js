...
window.calculateFreeTimeSlots = function(selectedDays) {
    ...
    const usableStart = timeStrToMinutes(scheduleData?.dayWindow?.start || '07:00');
    const usableEnd = timeStrToMinutes(scheduleData?.dayWindow?.end || '23:00');

    selectedDays.forEach(dayName => {
        const day = dayEntries.find(entry => entry?.name === dayName);
        const dayBlocks = day?.blocks || [];
        const busyIntervals = [];
        dayBlocks.forEach(block => {
            busyIntervals.push(...getBlockIntervalsWithinWindow(block, usableStart, usableEnd));
        });
        const mergedBusy = mergeIntervals(busyIntervals);
        const gaps = [];
