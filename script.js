...
function loadHomeInventory() {
    try {
        ...
        return parsed.map(raw => {
            const unlimited = Boolean(raw.unlimited) || (raw.shop === 'Tap' && (raw.qtyUnits === null || raw.qtyUnits === undefined || raw.qtyUnits === 'unlimited'));
            const qtyUnitsRaw = raw.qtyUnits ?? raw.qtyAvailable ?? 0;
...
function collectHomeChecklistSelections(shop) {
    ...
    rows.forEach(row => {
        ...
        const qtyUnits = isTap ? null : parseFloat(qtyInput?.value);
        if (isTap || (!isNaN(qtyUnits) && qtyUnits > 0)) {
            selections.push({
                ...
                unit,
                price,
                qtyAvailablePacks: isTap ? null : qtyUnits / packSize,
                qtyUnits: isTap ? null : qtyUnits,
                unitLabel: unit,
                unlimited: isTap
            });
        }
