...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const staffMeal = document.getElementById('promptStaffMeal');
    const workMealPlan = document.getElementById('promptWorkMealPlan');
    if (cookingFor) {
    ...
    if (staffMeal) {
        staffMeal.addEventListener('change', updateWorkMealVisibility);
    }
    if (workMealPlan) {
        workMealPlan.addEventListener('change', updateWorkMealVisibility);
    }
...
function generateWeekFromForm() {
    try {
        ...
        // Extract key inputs
        const weekStartStr = formData.get('promptWeekDate');
        if (!weekStartStr) {
            alert('⚠️ Please select a week start date.');
            return;
        }
        ...
        const shoppingAnchorDayRaw = shoppingAnchorRaw === 'custom' ? shoppingAnchorCustom : parseInt(shoppingAnchorRaw, 10);
        ...
        const staffMeal = form.querySelector('#promptStaffMeal')?.checked !== false; // default true
        const workMealPlan = formData.get('promptWorkMealPlan') || 'cook'; // cook | buy
        const workMealPrep = formData.get('promptWorkMealPrep') || 'evening'; // evening | morning | any
        const workMealPrepMinutes = parseInt(formData.get('promptWorkMealPrepMinutes') || '30', 10);
        const workMealCount = parseInt(formData.get('promptWorkMealCount') || '2', 10); // 1 or 2
        const lateShiftNoCook = form.querySelector('#promptLateShiftNoCook')?.checked;
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if ((workStartStr && !workEndStr) || (!workStartStr && workEndStr)) {
                alert(`⚠️ Work time missing start/end on ${dayName}. Please provide both or leave blank.`);
                return;
            }
            ...
            const placeWork = () => {
                if (hasWork && workStart != null && workEnd != null) {
                    ...
                    const breakStart = workStart + Math.max(60, Math.floor(workDuration / 2) - 15);
                    const breakEnd = breakStart + breakDuration;

                    if (staffMeal) {
                        addBlock(blocks, breakStart, breakEnd, '🍽️ Staff meal break');
                    } else if (workMealPlan === 'buy') {
                        addBlock(blocks, breakStart, breakEnd, '🍔 Buy/Order meal break');
                    } else { // cook/bring packed meal
                        // Determine prep time slot
                        const prepMinutes = isNaN(workMealPrepMinutes) ? 30 : workMealPrepMinutes;
                        let prepStart = workStart - prepMinutes - (addCommute ? commuteDuration + commutePrep : 0);
                        let prepEnd = prepStart + prepMinutes;

                        if (workMealPrep === 'evening') {
                            prepStart -= 12 * 60; // push to previous evening approx
                            prepEnd -= 12 * 60;
                        }
                        if (lateShiftNoCook && workEnd >= 21 * 60) {
                            // Avoid cooking after late shift; ensure prep is before shift
                            prepEnd = Math.min(prepEnd, workStart);
                            prepStart = prepEnd - prepMinutes;
                        }
                        addBlock(blocks, prepStart, prepEnd, '🥪 Prep/pack meal for work', `${prepMinutes}m prep`);
                        addBlock(blocks, breakStart, breakEnd, '🍱 Packed meal break');
                        if (workMealCount > 1) {
                            const secondBreakStart = breakEnd + 120; // later snack/dinner
                            addBlock(blocks, secondBreakStart, secondBreakStart + breakDuration, '🍱 Packed meal (2nd)');
                        }
                    }
                }
            };
            ...
            // Apply priority presets
            if (priorityPreset === 'meals') {
                placeMeals();
                placeWork();
                placeStudy();
                placeTasks();
                placeHobbies();
            } else if (priorityPreset === 'errand') {
                placeWork();
                placeTasks();
                placeStudy();
                placeHobbies();
...
function updateWorkMealVisibility() {
    const staffMeal = document.getElementById('promptStaffMeal');
    const planRow = document.getElementById('promptWorkMealPlanRow');
    const prepRow = document.getElementById('promptWorkMealPrepRow');
    const countRow = document.getElementById('promptWorkMealCountRow');

    const staffChecked = staffMeal ? staffMeal.checked : true;
    if (planRow) planRow.style.display = staffChecked ? 'none' : 'block';

    const plan = document.getElementById('promptWorkMealPlan')?.value || 'cook';
    const showPrep = !staffChecked && plan === 'cook';
    if (prepRow) prepRow.style.display = showPrep ? 'block' : 'none';
    if (countRow) countRow.style.display = staffChecked ? 'none' : 'block';
}
...
console.log('✅ Phase 6: Settings & History loaded!');
        // Workday meal handling
        const staffMeal = form.querySelector('#promptStaffMeal')?.checked;
        const workMealPlan = formData.get('promptWorkMealPlan') || 'cook'; // cook | buy
        const workMealPrep = formData.get('promptWorkMealPrep') || 'evening'; // evening | morning | any
        const workMealPrepMinutes = parseInt(formData.get('promptWorkMealPrepMinutes') || '30', 10);
        const workMealCount = parseInt(formData.get('promptWorkMealCount') || '2', 10); // 1 or 2
