...
function loadHomeInventory() {
    try {
        ...
        return parsed.map(raw => {
            ...
            const unitLabel = raw.unitLabel || raw.unit || '';
            const sanitizedQty = unlimited ? Infinity : sanitizeQuantityForUnit(qtyUnitsRaw, unitLabel, true);
    ...
    Object.entries(grouped).forEach(([category, items]) => {
        ...
        items.forEach(item => {
            ...
            const qtyVal = saved?.qtyUnits === Infinity || saved?.unlimited
                ? ''
                : (typeof saved?.qtyUnits === 'number' ? formatQuantityForDisplay(saved.qtyUnits, item.unit) : '');
            ...
            if (isTap || saved?.unlimited) {
                ...
                qtyRow.innerHTML = `
                    ...
                    <div style="display: flex; align-items: center; gap: 6px;">
                        <input type="number" class="home-qty-input" min="0" ${unitAllowsDecimals(item.unit) ? 'step="0.01"' : 'step="1"'} placeholder="${item.packSize || 1}" value="${qtyVal}" style="padding: 8px 10px; border: 1px solid #d1d5db; border-radius: 8px; width: 90px;">
            ...
            const qtyInput = card.querySelector('.home-qty-input');
            if (qtyInput) {
                applyQuantityInputRules(qtyInput, item.unit);
            }
        });
...
function collectHomeChecklistSelections(shop) {
    ...
    cards.forEach(card => {
        ...
        const qtyUnits = unlimited ? null : sanitizeQuantityForUnit(rawValue, unit);
