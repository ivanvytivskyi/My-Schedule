...
function packMorningBlocksBeforeWork(blocks, workStartTime) {
    ...
    // Day start at midnight for packing
    let cursor = 0;
    const available = workStartMins - cursor;
    if (available <= 0) return blocks;
    
    // Select morning blocks: those starting before work and not the work block itself
    const morningBlocks = blocks.filter(b => {
        const rng = getBlockTimeRange(b);
        return !isWorkBlock(b) && !isNaN(rng.start) && rng.start < workStartMins;
    });
    
    if (morningBlocks.length === 0) return blocks;
    
    // Preserve original order by start time
    morningBlocks.sort((a, b) => {
    ...
    let packedBlocks = [];
    let consumed = 0;
    morningBlocks.forEach((block, idx) => {
        const duration = getBlockDurationMinutes(block);
        const start = cursor + consumed;
        let end = start + duration;
        
        if (end > workStartMins) {
            end = workStartMins;
        }
        
        const packed = { ...block, time: `${formatMinutesToTime(start)}-${formatMinutesToTime(end)}` };
        packedBlocks.push(packed);
        consumed += (end - start);
    });
    
    // If overflow, trim last block to fit
    if (consumed > available && packedBlocks.length > 0) {
        const overBy = consumed - available;
        const last = packedBlocks[packedBlocks.length - 1];
        const lastRange = getBlockTimeRange(last);
        const newEnd = Math.max(lastRange.start, lastRange.end - overBy);
        last.time = `${formatMinutesToTime(lastRange.start)}-${formatMinutesToTime(newEnd)}`;
    }
    
    // Replace originals in-place
    const packedSet = new Set(morningBlocks);
    const result = blocks.map(b => {
        if (!packedSet.has(b)) return b;
        const replacement = packedBlocks.shift();
