...
function renderHomeInventoryTable() {
    ...
    const rows = inventory.map(item => {
        ...
        const readableUnit = formatHomeUnitLabel(unitLabel, item.unlimited ? null : item.qtyUnits);
        ...
        return `
            <tr>
                ...
                <td style="border: 1px solid #e5e7eb; padding: 10px;">${readableUnit}</td>
...
function renderHomeInventoryChecklist(shop, savedInventory) {
    ...
    Object.entries(grouped).forEach(([category, items]) => {
        ...
        items.forEach(item => {
            ...
            const defaultHomeUnit = item.homeUnit || meta.homeUnit;
            ...
            const trackingOptions = isBread ? ['g', 'each'] : [defaultHomeUnit];
            ...
            const qtyVal = saved?.qtyUnits === Infinity || saved?.unlimited
                ? ''
                : (typeof saved?.qtyUnits === 'number' ? formatQuantityForDisplay(saved.qtyUnits, saved?.homeUnit || selectedHomeUnit) : '');
            ...
            if (qtyVal && saved?.homeUnit && saved.homeUnit !== selectedHomeUnit && typeof saved.qtyUnits === 'number') {
                ...
                if (savedGramsPerSlice && isFinite(savedGramsPerSlice)) {
                    if (saved.homeUnit === 'g' && selectedHomeUnit === 'each') {
                        convertedVal = saved.qtyUnits / savedGramsPerSlice;
                    } else if (saved.homeUnit === 'each' && selectedHomeUnit === 'g') {
                        convertedVal = saved.qtyUnits * savedGramsPerSlice;
                    }
                }
                if (convertedVal === null) {
                    convertedVal = convertUnitsToTarget(saved.qtyUnits, saved.homeUnit, selectedHomeUnit);
                }
            ...
            topRow.innerHTML = `
                ...
                <div style="flex: 1;">
                    ...
                    <div style="color: #6b7280; font-size: 12px;">Sold as: ${packUnit} · Qty at home = what's left (${selectedHomeUnit === 'each' ? (isBread ? 'slices' : 'pieces') : selectedHomeUnit})</div>
                </div>
            ...
            if (isTap || saved?.unlimited) {
                ...
                const homeUnitLabel = selectedHomeUnit === 'each'
                    ? (isBread ? 'slices' : 'each')
                    : selectedHomeUnit;
                ...
                if (selectedHomeUnit === 'each' && unitsPerPack) {
                ...
                qtyInput.addEventListener('input', () => {
                    ...
                    if (unitsPerPack && val > unitsPerPack) {
                ...
                unitBadge.textContent = selectedHomeUnit === 'each' ? (isBread ? 'slices' : 'each') : selectedHomeUnit;
                ...
                const updateHelperText = () => {
                    ...
                    const unitHint = selectedHomeUnit === 'each' ? (isBread ? 'Enter slices left' : 'Enter pieces left') : `Track ${selectedHomeUnit} left`;
                ...
                if (trackingOptions.length > 1) {
                    ...
                    trackSelect.addEventListener('change', () => {
                        ...
                        qtyInput.step = getQuantityStep(selectedHomeUnit);
                        applyQuantityInputRules(qtyInput, selectedHomeUnit);
                        qtyLabel.textContent = `Qty left (${selectedHomeUnit === 'each' ? 'slices' : selectedHomeUnit}):`;
                        unitBadge.textContent = selectedHomeUnit === 'each' ? 'slices' : selectedHomeUnit;
                        updateHelperText();
                        ...
                        if (previousUnit !== selectedHomeUnit && qtyInput.value) {
                            ...
                            if (gramsPerSlice && isFinite(gramsPerSlice)) {
                                if (previousUnit === 'g' && selectedHomeUnit === 'each') {
                                    converted = qtyInput.value ? (parseFloat(qtyInput.value) / gramsPerSlice) : null;
                                } else if (previousUnit === 'each' && selectedHomeUnit === 'g') {
                                    converted = qtyInput.value ? (parseFloat(qtyInput.value) * gramsPerSlice) : null;
                                }
                            }
                            if (converted === null) {
                        ...
                        if (gramsInputWrapper) {
                            gramsInputWrapper.style.display = selectedHomeUnit === 'each' ? 'flex' : 'none';
                        }
                    });
                ...
                if (trackingOptions.length > 1) {
                    ...
                    gramsInputWrapper.style.cssText = `display: ${selectedHomeUnit === 'each' ? 'flex' : 'none'}; align-items: center; gap: 6px;`;
...
function collectHomeChecklistSelections(shop) {
    ...
    cards.forEach(card => {
        ...
        const effectiveUnitsPerPack = (!unlimited && homeUnit === 'each' && gramsPerSlice && isFinite(gramsPerSlice) && trackingOptions.includes('each'))
            ? (unitsPerPack / gramsPerSlice)
            : unitsPerPack;
        const storedUnitsPerPack = effectiveUnitsPerPack || unitsPerPack || 1;
        ...
        if (unlimited || qtyUnits > 0) {
            selections.push({
                ...
                unit: packUnit,
                price,
                qtyAvailablePacks: unlimited ? 'unlimited' : qtyUnits / storedUnitsPerPack,
