...
function isShoppingChainBlock(block) {
...
function packMorningBlocksBeforeWork(blocks, workStartTime) {
    ...
    // Pack contiguously backward from commute/work anchor, no overlaps
...
function enforceBreakfastBeforeCommute(blocks, workStartTime, date) {
    if (!workStartTime) return blocks;
    const workStartMins = timeStrToMinutes(workStartTime);
    if (isNaN(workStartMins)) return blocks;

    const commuteAnchor = blocks.reduce((min, block) => {
        if (isCommuteToWorkBlock(block) || isCommutePrepBlock(block)) {
            const { start } = getBlockTimeRange(block);
            if (!isNaN(start)) return Math.min(min, start);
        }
    ...
    const anchorStart = Number.isFinite(commuteAnchor) ? commuteAnchor : workStartMins;

    const breakfastBlock = blocks.find(block => mealTypeOf(block) === 'breakfast' && !block.isCookingBlock);
    if (!breakfastBlock) return blocks;

    const anchorId = breakfastBlock.id || breakfastBlock.linkedBlockId;
    const chainBlocks = blocks.filter(block =>
        block?.id === anchorId || block?.linkedBlockId === anchorId || block === breakfastBlock
    );
    const chainRanges = chainBlocks
        .map(chainBlock => ({ block: chainBlock, ...getBlockTimeRange(chainBlock) }))
        .filter(range => !isNaN(range.start) && !isNaN(range.end));
    if (!chainRanges.length) return blocks;

    const chainStart = Math.min(...chainRanges.map(range => range.start));
    const chainEnd = Math.max(...chainRanges.map(range => range.end));

    if (chainEnd <= anchorStart) return blocks;
    let shiftBy = anchorStart - chainEnd;
    if (chainStart + shiftBy < 0) {
        shiftBy = -chainStart;
    }
...
function packEveningBlocksAfterWork(blocks, workEndTime) {
    ...
    const candidates = blocks.filter(b => {
        if (isWorkBlock(b) || isWorkCommuteBlock(b) || isShoppingChainBlock(b) || isMealOrCookingBlock(b)) {
            return false;
        }
        const title = (b.title || '').toLowerCase();
...
async function addWeek(options = {}) {
    ...
    function isFixedBlockForMeals(block) {
        ...
        if (block.type === 'shopping' || block.type === 'travel-shopping' || block.type === 'shopping-prep' || block.type === 'shopping-unpack') return true;
        ...
        if (isWorkDay) {
            ...
            if (isMorningShift) {
                ...
                if (needsPacking) {
                    ...
                    blocks = tightenMealChainsToPreviousBlock(blocks, date);
                    blocks = enforceBreakfastBeforeCommute(blocks, workSchedule[dayOfWeek].start, date);
                } else {
                    const availableTime = startMins - earliestDefaultStart;
...
function renderSchedule() {
    ...
    if (!currentDay || !scheduleData.days[currentDay]) {
        const sortedDays = Object.entries(scheduleData.days).sort((a, b) => new Date(a[1].date) - new Date(b[1].date));
        const firstDayKey = sortedDays[0]?.[0];
        if (firstDayKey) {
            currentDay = firstDayKey;
        }
    }
    const hasActive = document.querySelector('.day-content.active');
    if (currentDay && !hasActive) {
        showDay(currentDay);
...
function isLinkedToFixedBlock(block, blocks) {
    ...
    return chainBlocks.some(item =>
        item.fixedTime === true || isWorkBlock(item) || isStudyBlock(item) || isShoppingChainBlock(item)
    );
}
...
function isFixedTimeBlock(block, blocks = []) {
    ...
    if (isShoppingChainBlock(block)) return true;
...
function applyPriorityNudges(blocks, date, options = {}) {
    ...
    const chainSnapshots = chains.map(chain => {
        ...
        const hasShoppingChain = chain.blocks.some(block => isShoppingChainBlock(block));
        const fixed = chain.blocks.some(block => isFixedTimeBlock(block, blocks)) || hasMealChain || hasShoppingChain;
        return { ...chain, start, end, fixed, hasMealChain, hasShoppingChain };
...
function dedupeSleepBlocks(blocks) {
    ...
    blocks.forEach(block => {
        ...
        if (block.startDateTime && block.endDateTime) {
            ...
            const hasValidDateTimes = !Number.isNaN(startTime) && !Number.isNaN(endTime);
            if (hasValidDateTimes) {
                ...
                const existingIndex = sleepBlocks.findIndex(existing => {
                    ...
                    if (Number.isNaN(existingStartTime) || Number.isNaN(existingEndTime)) return false;
