...
function applyWorkScheduleToOrderedDays(orderedDays) {
    ...
    const addCommute = usePatternCommute || document.getElementById('addCommuteNew')?.checked || document.getElementById('addCommute')?.checked;
    const commuteDurationRaw = usePatternCommute
        ? patternCommute
    ...
    const addCommutePrep = addCommute && (usePatternCommute || document.getElementById('addCommutePrepNew')?.checked || false);
    const commutePrepRaw = usePatternCommute
        ? patternPrep
        : parseInt(document.getElementById('commutePrepDurationNew')?.value || '0', 10);
...
function renderSetupShoppingNeeded() {
    const container = document.getElementById('setupShoppingContainer');
    if (!container) return;
    ...
    if (!setupWeekCreated) {
        container.innerHTML = `
            <div class="setup-shopping-card">
                ...
                <p>Create your week to see shopping timing and available slots.</p>
            </div>
        `;
        return;
    }
    ...
    if (dayEntries.length === 0) {
        container.innerHTML = `
            <div class="setup-shopping-card">
                <h3>🛒 Shopping Needed</h3>
                <p>No schedule yet. Add at least one day to see shopping timing and free slots.</p>
            </div>
        `;
        return;
    }

    const orderedDays = dayEntries.map(day => ({
        ...
        <div class="setup-shopping-card">
            ...
            <div class="setup-shopping-slots">
                ...
                <div id="setupShoppingSlotsEmpty" class="setup-shopping-empty" style="display: none;">
                    No free time slots found before ${needsFromDate}.
                </div>
            </div>
            ...
            <div class="setup-step-actions">
                <button class="btn btn-primary" type="button" id="setupAddShoppingBlocks">Add shopping blocks</button>
            </div>
        </div>
    `;
    ...
    const addButton = container.querySelector('#setupAddShoppingBlocks');
    let availableSlots = [];
        ...
        slotsList.innerHTML = availableSlots.map((slot, idx) => {
            ...
            return `
                <label class="setup-shopping-slot-option">
                    <input type="radio" name="setupShoppingSlot" value="${idx}" />
    ...
    addButton.addEventListener('click', () => {
        ...
        const selectedRadio = container.querySelector('input[name="setupShoppingSlot"]:checked');
        if (!selectedRadio) {
        ...
        const sanitizedUnpack = Number.isFinite(unpackMins) && unpackMins >= 0 ? unpackMins : 10;
        const totalDuration = Math.max(15, sanitizedPrep + (sanitizedTravel * 2) + sanitizedShop + sanitizedUnpack);
        const startInput = container.querySelector(`#setupShoppingStart_${slotIndex}`);
        ...
        if (success) {
            ...
            if (typeof showToast === 'function') {
                showToast('🛒 Shopping blocks added to your week!');
            }
        }
    });
}
...
async function addWeek(options = {}) {
    ...
    const usePatternCommute = patternMode === 'same' && patternRows.length;
    const addCommute = usePatternCommute || document.getElementById('addCommuteNew')?.checked || document.getElementById('addCommute').checked;
    const commuteDuration = addCommute ? (usePatternCommute ? patternCommute : parseInt(document.getElementById('commuteDurationNew')?.value || document.getElementById('commuteDuration')?.value)) : 0;
    const addCommutePrep = addCommute && (usePatternCommute || document.getElementById('addCommutePrepNew')?.checked || false);
    const commutePrepDuration = addCommutePrep ? (usePatternCommute ? patternPrep : parseInt(document.getElementById('commutePrepDurationNew')?.value || '0') || 0) : 0;
