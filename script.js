...
function addWeek() {
    ...
    if (addWorkSchedule) {
        // Prefer new UI inputs (day-work-start/day-work-end) if present
        const dayNumbers = { sunday: 0, monday: 1, tuesday: 2, wednesday: 3, thursday: 4, friday: 5, saturday: 6 };
        const newStarts = document.querySelectorAll('.day-work-start');
        let capturedFromNewUI = false;
        if (newStarts && newStarts.length > 0) {
            newStarts.forEach(startInput => {
                const dayKey = startInput.dataset.day;
                const endInput = document.querySelector(`.day-work-end[data-day="${dayKey}"]`);
                if (!dayKey || !endInput) return;
                const startTime = startInput.value;
                const endTime = endInput.value;
                if (startTime && endTime && dayNumbers.hasOwnProperty(dayKey)) {
                    const dayValue = dayNumbers[dayKey];
                    workSchedule[dayValue] = { start: startTime, end: endTime };
                    capturedFromNewUI = true;
                }
            });
        }
        
        // Fallback to legacy checkbox-driven inputs if nothing captured
        if (!capturedFromNewUI) {
