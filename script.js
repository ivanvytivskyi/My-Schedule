...
function enforceLinkedBlockChains(blocks, date) {
    ...
    chainMap.forEach(chainBlocks => {
        ...
        const originalStart = ordered.reduce((min, block) => {
            const { start } = getBlockTimeRange(block);
        ...
        if (!isFinite(originalStart) || !chainDuration) return;
        ...
        let targetGap = gaps.find(gap => originalStart >= gap.start && (originalStart + chainDuration) <= gap.end);
        if (!targetGap) {
            targetGap = gaps.find(gap => gap.start >= originalStart && gap.duration >= chainDuration);
        }
        if (!targetGap) {
            const backwards = gaps.filter(gap => gap.end <= originalStart && gap.duration >= chainDuration);
            targetGap = backwards.length ? backwards[backwards.length - 1] : null;
        }
        if (!targetGap) return;

        const conflicts = otherBlocks.filter(block => {
