...
function setupModeButtons() {
    ...
    calendarBtn.addEventListener('click', () => {
        ...
        hideEditElements();
        manageDefaultsBtn.style.display = 'none';
    });
    ...
    editBtn.addEventListener('click', () => {
        ...
        showEditElements();
        manageDefaultsBtn.style.display = 'inline-block';
    });
...
function setupDateEditing() {
    ...
    dateDisplay.textContent = scheduleData.dateInfo;
    editableDate.value = scheduleData.dateInfo;

    editableDate.addEventListener('change', () => {
        scheduleData.dateInfo = editableDate.value;
        dateDisplay.textContent = editableDate.value;
        saveToLocalStorage();
    });
}
...
function applyTimePicker() {
    ...
    document.getElementById('blockTime').value = `${startHour}:${startMinute}-${endHour}:${endMinute}`;
...
function addNewBlock(dayKey, afterIndex) {
    ...
    if (afterIndex >= 0 && scheduleData.days[dayKey].blocks[afterIndex]) {
        ...
        const [hours, minutes] = previousEndTime.split(':');
...
function updateNavButtons() {
    ...
    if (activeTab && container) {
        ...
        // Calculate position to center the tab (with -300 offset to show more right tabs)
        // Center position = tab's left edge + half tab width - half container width
        const centerPosition = tabLeft + (tabWidth / 2) - (containerWidth / 2) - 300;
        ...
        console.log('Centering tab:', activeIndex, 'Position:', centerPosition);
    }
}
...
function renderShopping() {
    ...
    // Load saved shopping text from localStorage
    const savedText = localStorage.getItem('shoppingListText');
    
    if (!savedText) {
        container.innerHTML = `
            <div style="text-align: center; color: #999; padding: 40px 20px;">
                <p style="font-size: 18px; margin: 0;">📝 No shopping list yet</p>
                <p style="font-size: 14px; margin: 10px 0 0 0;">Use "🛒 Import Shopping" to add your list</p>
            </div>
        `;
    } else {
...
function openDefaultsModal() {
    ...
    document.getElementById('manageDefaultsModal').classList.add('active');
}

function closeDefaultsModal() {
    document.getElementById('manageDefaultsModal').classList.remove('active');
}
    ...
    for (let i = 0; i < 7; i++) {
        ...
        html += `
            <div style="background: white; padding: 12px; border-radius: 8px; display: flex; align-items: center; gap: 12px;">
                <span style="font-weight: 600; min-width: 100px; color: #2c3e50;">${dayName}</span>
                <span style="color: #7f8c8d; font-size: 13px; min-width: 60px;">${dateStr}</span>
                <input type="time" id="workDay${i}Start" placeholder="--:--" style="padding: 8px; border: 2px solid #ddd; border-radius: 6px; font-size: 14px;" />
                <span style="color: #7f8c8d;">to</span>
                <input type="time" id="workDay${i}End" placeholder="--:--" style="padding: 8px; border: 2px solid #ddd; border-radius: 6px; font-size: 14px;" />
            </div>
        `;
    }
