...
function loadHomeInventory() {
    try {
        ...
        return parsed.map(raw => {
            ...
            const qtyUnits = unlimited ? Infinity : (isNaN(parseFloat(qtyUnitsRaw)) ? 0 : parseFloat(qtyUnitsRaw));
            
            return {
                shop: raw.shop,
                category: raw.category,
                itemName: raw.itemName,
                unit: raw.unit || raw.unitLabel || '',
                price: raw.price,
                qtyAvailablePacks: unlimited ? Infinity : (raw.qtyAvailablePacks ?? raw.qtyAvailable ?? null),
                qtyUnits,
                unitLabel: raw.unitLabel || raw.unit || '',
                unlimited
            };
        });
    } catch (e) {
...
function renderHomeInventoryTable() {
    ...
    const rows = inventory.map(item => {
        const qtyDisplay = item.unlimited || item.qtyUnits === Infinity
            ? 'Unlimited'
            : item.qtyUnits
                ? `${item.qtyUnits} ${item.unitLabel || ''}`.trim()
                : item.qtyAvailablePacks
                    ? `${Number(item.qtyAvailablePacks).toFixed(2)} packs`
                    : '0';
        return `
            <tr>
...
function renderHomeInventoryChecklist(shop, savedInventory) {
    ...
    Object.entries(grouped).forEach(([category, items]) => {
        ...
        items.forEach(item => {
            ...
            const qtyVal = saved?.qtyUnits === Infinity || saved?.unlimited ? '' : (saved?.qtyUnits || '');
            ...
            card.dataset.packsize = item.packSize;
            ...
            topRow.innerHTML = `
                ...
                <div style="flex: 1;">
                    ...
                    <div style="color: #6b7280; font-size: 12px;">${item.unit} · ${category}</div>
                </div>
                <div style="padding: 6px 10px; background: #f3f4f6; border-radius: 8px; font-weight: 700; color: #374151; font-size: 12px;">${item.unit}</div>
            `;
            ...
            if (isTap || saved?.unlimited) {
                ...
                qtyRow.innerHTML = `
                    ...
                    <div style="display: flex; align-items: center; gap: 6px;">
                        <input type="number" class="home-qty-input" min="0" step="0.01" placeholder="${item.packSize || 1}" value="${qtyVal}" style="padding: 8px 10px; border: 1px solid #d1d5db; border-radius: 8px; width: 90px;">
...
function collectHomeChecklistSelections(shop) {
    ...
    cards.forEach(card => {
        ...
        const packSize = parseFloat(card.dataset.packsize) || 1;
        ...
        const qtyUnits = unlimited ? null : parseFloat(qtyInput?.value);
        if (unlimited || (!isNaN(qtyUnits) && qtyUnits > 0)) {
            selections.push({
                shop,
                category,
                itemName,
                unit,
                price,
                qtyAvailablePacks: unlimited ? 'unlimited' : qtyUnits / packSize,
