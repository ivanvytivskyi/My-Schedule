...
function applyWorkScheduleToOrderedDays(orderedDays) {
    ...
    const hasSharedCommute = Number.isFinite(sharedCommute) || Number.isFinite(sharedPrep);
...
function renderSetupShoppingNeeded() {
    ...
    const renderDayButtons = () => {
        daysContainer.innerHTML = orderedDays.map((day, idx) => {
            const dateLabel = day.date.toLocaleDateString('en-US', { weekday: 'long' });
...
function embedAdjustableWorkSchedule() {
    ...
    const commuteToggle = container.querySelector('#addCommuteNew')?.closest('label');
    if (commuteToggle) {
