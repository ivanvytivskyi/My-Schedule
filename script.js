    ...
    orderedDays.forEach(({ date }, i) => {
        ...
        if (isWorkDay) {
            ...
            if (!isNaN(shiftStart) && !isNaN(shiftEnd)) {
                ...
                const maxGapAfterShift = maxGapInWindow(blocksWithoutDinner, shiftEnd, dayEndMinutes);

                const keepLunch = maxGapBeforeShift >= lunchMin;
                // Late shifts (start >= 16:00) should not have dinner inserted
                const keepDinner = shiftStart >= 16 * 60 ? false : maxGapAfterShift >= dinnerMin;
                ...
                blocks = blocks.filter(block => {
                    ...
                    if (dinner) {
                        ...
                        const dinnerStartsAfterShift = !isNaN(start) && start >= shiftStart;
                        if (!keepDinner || !dinnerStartsAfterShift) return false;
                    }
                    return true;
                });
            }
