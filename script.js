...
function renderSetupShoppingNeeded() {
    ...
    const slots = findAvailableShoppingSlots(orderedDays, needsFrom, 90, 2);
    const slotsHtml = slots.length
        ? slots.map(slot => `
            <div class="setup-shopping-slot">
                <div class="slot-title">${slot.dateLabel}</div>
                <div class="slot-time">${slot.start} – ${slot.end}</div>
                <div class="slot-meta">${slot.description}</div>
            </div>
        `).join('')
        : `<div class="setup-shopping-empty">No free time slots found before ${needsFromDate}.</div>`;
