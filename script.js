...
// Calculate leftover allowance based on recipe serves (ISSUE 5)
function getLeftoverAllowance(recipe) {
    if (!recipe || !recipe.serves) return 0;
    
    const serves = parseInt(recipe.serves);
    
    // Serves 6-8 → max 3 leftover meals
    if (serves >= 6) return 3;
    
    // Serves 4-5 → max 2 leftover meals  
    if (serves >= 4) return 2;
    
    // Serves <4 → max 1 leftover meal (or 0 if very small)
    if (serves >= 2) return 1;
    
    return 0; // Single serving = no leftovers
}
...
async function addWeek() {
    ...
    for (let i = 0; i < orderedDays.length; i++) {
        ...
        // Skip lunch/dinner blocks on work days
        if (isWorkDay) {
            blocks = blocks.filter(b => !isWorkMealBlock(b));
        }
        ...
        if (isWorkDay) {
            // Find work block
            const workBlock = blocks.find(b => {
                const title = (b.title || '').toLowerCase();
                return title.includes('💼 work') || title.includes('work');
            });
            
            if (workBlock) {
                const workRange = getBlockTimeRange(workBlock);
                
                blocks = applyWorkMealRules(blocks, workRange);
...
function renderSchedule() {
    ...
    for (const dayKey in scheduleData.days) {
        ...
        html += `
            <div class="day-content" id="${dayKey}Content">
                <div class="day-header">
                    <div class="day-free-time" id="freeTime-${dayKey}">✨ Time for you: --</div>
        ...
        blocksWithIndices.forEach((item, displayIndex) => {
            ...
            const blockEnd = range.end < range.start ? range.end + 24 * 60 : range.end;

            if (!isNaN(blockStart) && !isNaN(previousEnd)) {
                if (shouldRenderAddButton(previousEnd, blockStart, workRange)) {
        ...
        if (!isNaN(previousEnd) && shouldRenderAddButton(previousEnd, safeDayEnd, workRange)) {
            html += `<div class="add-block-container">
                <button class="add-block-btn" onclick="addNewBlock('${dayKey}', ${lastOriginalIndex})">➕</button>
            </div>`;
        }
...
function updateAllDayFreeTime() {
    for (const dayKey in scheduleData.days) {
        ...
        el.textContent = `✨ Time for you: ${formatMinutesAsHoursAndMinutes(freeMins)}`;
    }
}

function shouldRenderAddButton(gapStart, gapEnd, workRange) {
    ...
    if (!isNaN(workStart) && !isNaN(workEnd)) {
        if (gapStart >= workStart && end <= workEnd) return false;
    }
    return true;
}
...
function applyWorkMealRules(blocks, workRange) {
    if (!workRange) return blocks;
    const workStart = workRange.start;
    const workEnd = workRange.end;
    if (isNaN(workStart) || isNaN(workEnd)) return blocks;
...
function takeRecipeForCategory(category) {
    ...
    const recipe = mealRecipeQueues[category].shift();
    console.log(`   ✅ Took from ${category} queue: ${recipe?.id} (${recipe?.name}), ${mealRecipeQueues[category].length} remaining`);
    return recipe;
}
