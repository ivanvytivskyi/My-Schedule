...
function packMorningBlocksBeforeWork(blocks, workStartTime) {
    ...
    // Preserve original order (by start time) and pack forward from midnight
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (end > workStartMins) {
            end = workStartMins; // trim only when necessary on the last block
        }
        
        const packed = { ...block, time: `${formatMinutesToTime(start)}-${formatMinutesToTime(end)}` };
        packedMap.set(block, packed);
        cursor = end;
    }
    
    return blocks.map(b => packedMap.get(b) || b);
}
