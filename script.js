    ...
    chainSnapshots.forEach(chain => {
        if (cursor !== null && chain.start < cursor) {
            ...
            if (!chain.fixed && (shiftBy <= maxShiftMinutes || chain.hasMealChain)) {
                ...
                chain.blocks.forEach(block => {
                    if (!isFixedTimeBlock(block, blocks) && !isMealOrCookingBlock(block)) conflicts.add(block);
                });
            }
        }
    ...
    for (let i = 0; i < sortedBlocks.length; i += 1) {
        ...
        for (let j = i + 1; j < sortedBlocks.length; j += 1) {
            ...
            if (current.start < next.end && current.end > next.start) {
                if (!isFixedTimeBlock(current.block, blocks) && !isMealOrCookingBlock(current.block)) conflicts.add(current.block);
                if (!isFixedTimeBlock(next.block, blocks) && !isMealOrCookingBlock(next.block)) conflicts.add(next.block);
            }
        }
    }
