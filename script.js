...
function packEveningBlocksAfterWork(blocks, workEndTime) {
    ...
    for (let i = 0; i < descriptors.length; i++) {
        ...
        if (start >= minutesInDay) {
            carryOver.push({ ...d.ref, time: `00:00-${formatMinutesToTime(end % minutesInDay)}` });
            cursor = end;
        ...
        if (end > minutesInDay) {
            ...
            // Carry the next-day portion starting at 00:00
            carryOver.push({ ...d.ref, time: `00:00-${formatMinutesToTime(carryEnd)}` });
