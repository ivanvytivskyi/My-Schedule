...
function getFirstTimeGuideState() {
    return JSON.parse(localStorage.getItem(FIRST_TIME_GUIDE_STATE_KEY) || '{"currentStep":1,"completed":[]}');
}

function setFirstTimeGuideState(state) {
    localStorage.setItem(FIRST_TIME_GUIDE_STATE_KEY, JSON.stringify(state));
}
...
function markFirstTimeStepComplete(step) {
    ...
    if (state.currentStep === step) {
        state.currentStep = Math.min(4, step + 1);
    }
...
function updateFirstTimeGuideUI() {
    ...
    const stepText = {
        1: {
            ...
            body: 'Create your reusable daily blocks (morning routine, meals, gym, etc.). This makes every week auto‑fill fast.',
            primary: 'Open Daily Template',
            action: 'startFirstTimeStepOne'
        },
        2: {
            title: 'Step 2: Important Events',
            body: 'Add key events like appointments, exams, or deadlines so they appear at the top of your schedule.',
            primary: 'Add an Event',
            action: 'startFirstTimeStepTwo'
        },
        3: {
            title: 'Step 3: Work & Dates',
            body: 'Choose your week start date and optional work schedule so commute and meal times are planned correctly.',
            primary: 'Open Week Creator',
            action: 'startFirstTimeStepThree'
        },
        4: {
            title: 'Step 4: Create Your Week',
            body: 'You are ready to create the first week. Review your settings and press Create Week.',
            primary: 'Go to Week Creation',
...
function showFirstTimeGuideIfNeeded() {
    const hasDays = Object.keys(scheduleData.days || {}).length > 0;
    const dismissed = localStorage.getItem('firstTimeGuideDismissed') === 'true';
    if (hasDays || dismissed) return;
...
function startFirstTimeStepThree() {
    ...
    openAddDayModal();
    switchAddType('week');
    markFirstTimeStepComplete(3);
    updateFirstTimeGuideUI();
}

function startFirstTimeStepFour() {
    startFirstTimeStepThree();
}
