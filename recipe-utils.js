...
function randomBalancedSelection(allRecipes, targetCount = 18) {
    ...
    // Balanced distribution for 7-day week
    // Breakfast: 7 (variety for each day)
    // Batch: 2 (each lasts 2-3 days)
    // Main: 4 (quick meals)
    // Snacks: 2
    // Drink: 2
    // Rewards: 1
    ...
    console.log(`✅ Balanced selection: ${selected.length} recipes (first-time user)`);
 ...
 */
function selectRecipesForWeek(dietaryFilters = {}) {
    // Get all recipes
 ...
 * - Dynamically adjusts day window to earliest start / latest end across blocks
 * - Handles overnight blocks by treating them as ending at dayEnd for the current day
 */
function calculateTimeGaps(preFilledBlocks, defaultDayStart = "07:00", defaultDayEnd = "23:00") {
    const allDays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
    const gaps = {};
    ...
    // Determine dynamic bounds from all blocks across the week
    let globalEarliest = toMinutes(defaultDayStart);
    let globalLatest = toMinutes(defaultDayEnd);
    Object.values(preFilledBlocks || {}).forEach(dayBlocks => {
        (dayBlocks || []).forEach(block => {
            const startMin = toMinutes(block.start);
            let endMin = toMinutes(block.end);
            if (endMin < startMin) {
                endMin = toMinutes(defaultDayEnd); // overnight: clamp to default end for bound calc
            }
            globalEarliest = Math.min(globalEarliest, startMin);
            globalLatest = Math.max(globalLatest, endMin);
        });
    });
    
    const dayStartMin = globalEarliest;
    const dayEndMin = globalLatest;
